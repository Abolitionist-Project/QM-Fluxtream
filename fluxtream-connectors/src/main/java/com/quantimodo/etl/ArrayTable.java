...
public final class ArrayTable implements Table {
  ...
  private ArrayTable() { throw new UnsupportedOperationException("The default constructor is not valid."); }
  
  // Constructs ETLData from arrays.
  public ArrayTable(final String name, final String[] fieldNames, final Object[][] records) {
    this.name = name;
    
    if (fieldNames == null) throw new NullPointerException("No field names were provided");
    if (records    == null) throw new NullPointerException("No records were provided.");
    
    recordCount = records.length;
    fieldCount  = fieldNames.length;
    
    for (int i = 0; i < recordCount; i++) {
      if (records[i].length != fieldCount) throw new IllegalArgumentException("Record " + i + " doesn't have " + fieldCount + " fields.");
    }
  ...
  public int getFieldNumber(CharSequence fieldName) {
    ...
    for (int field = 0; field < fieldCount; field++) {
      if (fieldName.equals(fieldNames[field])) return field;
    }
    return -1;
  }
  ...
  public String getFieldName(final int field) {
    if ((field < 0) || (field >= fieldCount)) throw new ArrayIndexOutOfBoundsException("Field number " + field + " doesn't exist.");
  ...
  public boolean hasField(CharSequence fieldName) {
    fieldName = fieldName.toString();
    for (int field = 0; field < fieldCount; field++) {
      if (fieldName.equals(fieldNames[field])) return true;
    }
    return false;
  }
  ...
  public Object getData(final int record, final int field) {
    if ((record < 0) || (record >= recordCount)) throw new ArrayIndexOutOfBoundsException("Record number " + record + " doesn't exist.");
    if ((field  < 0) || (field  >= fieldCount )) throw new ArrayIndexOutOfBoundsException("Field number "  + field  + " doesn't exist.");
  ...
  public Object getData(final int record, final CharSequence fieldName) {
    ...
    if (field == -1) throw new ArrayIndexOutOfBoundsException("There is no field named " + fieldName + '.');
