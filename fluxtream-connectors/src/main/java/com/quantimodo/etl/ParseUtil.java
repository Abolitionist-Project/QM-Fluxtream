...
public final class ParseUtil {
  ...
  private ParseUtil() { throw new UnsupportedOperationException("Utility classes cannot be constructed."); }
  
  public static final Long parseNanoTime(final String nanoTime) {
    final int splitPosition = nanoTime.length() - 3;
    Date date = null;
    try { date = nanoTimeParser.parse(nanoTime.substring(0, splitPosition)); } catch (ParseException e) { return null; }
