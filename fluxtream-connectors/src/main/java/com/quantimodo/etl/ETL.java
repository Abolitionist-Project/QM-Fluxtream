...
public class ETL {
  ...
  public QuantimodoRecord[] handle(final String filename) throws IOException {
    return handle(new File(filename), false);
  }
  
  public QuantimodoRecord[] handle(final String filename, final boolean debugPrinting) throws IOException {
    return handle(new File(filename), debugPrinting);
  }
  
  public QuantimodoRecord[] handle(final File file) throws IOException {
    return handle(file, false);
  }
  
  public QuantimodoRecord[] handle(final File file, final boolean debugPrinting) throws IOException {
    if (file == null) throw new NullPointerException("ETL.handle was given null in place of a File");
    ...
    final DatabaseView database;
    {
      ...
      for (int i = 0; i < readers.length; i++) {
        try {
          ...
          if (readers[i] == SQLiteReader.instance) throw e;
        }
      }
      database = db;
    }
    if (database == null) throw new IOException("No Reader can decipher this file format.");
    if (debugPrinting) System.out.println(database);
    // System.out.println(java.util.Arrays.toString((byte []) database.getTable("records").getData(0, "recordData")));
    
    // Convert records
    final QuantimodoRecord[] outputRecords;
    {
      QuantimodoRecord[] recs = null;
    ...
    if (outputRecords == null) throw new IOException("No Converter understands this data." + database.toHTML());
    if (debugPrinting) {
      for (final QuantimodoRecord outputRecord : outputRecords) {
