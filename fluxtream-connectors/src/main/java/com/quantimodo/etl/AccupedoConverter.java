...
public class AccupedoConverter implements Converter {
  ...
  public QuantimodoRecord[] convert(final DatabaseView databaseView) {
    if ((databaseView == null) || (!databaseView.hasTable("diaries"))) return null;
    ...
    for (int recordNumber = 0; recordNumber < recordCount; recordNumber++) {
      ...
      if ((stepCount >= lastStepCount) && (lastDay == day) && (lastMonth == month) && (lastYear == year)) {
        ...
        else dayStartRecord = results.size();
      ...
      if (outputStepCount > 0) {
        if (outputAchievedGoal) results.add(new QuantimodoRecord("Accupedo", "goals", "daily step count goal reached", false, true, true, 1.0, "(count)", (new GregorianCalendar(year, month - 1, day)).getTimeInMillis(), 86400));
        results.add(new QuantimodoRecord("Accupedo", "activity", "steps"            , false, true,  true, outputStepCount, "steps", outputTime, (outputSteppingDuration + 500)/1000));
        results.add(new QuantimodoRecord("Accupedo", "activity", "walk/run distance", false, true,  true, outputDistance,  "mi",    outputTime, (outputSteppingDuration + 500)/1000));
        results.add(new QuantimodoRecord("Accupedo", "activity", "calories burned",   false, false, true, outputCalories,  "kcal",  outputTime, (outputSteppingDuration + 500)/1000));
      }
