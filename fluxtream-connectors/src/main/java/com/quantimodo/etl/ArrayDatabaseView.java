...
public class ArrayDatabaseView implements DatabaseView {
  ...
  private ArrayDatabaseView() { throw new UnsupportedOperationException("The default constructor is not valid."); }
  
  // Constructs ETLData from arrays.
  public ArrayDatabaseView(final String[] tableNames, final ArrayTable[] tables) {
    if (tableNames == null) throw new NullPointerException("No table names were provided");
    if (tables     == null) throw new NullPointerException("No tables were provided.");
  ...
  public int getTableNumber(CharSequence tableName) {
    ...
    for (int table = 0; table < tableCount; table++) {
      if (tableName.equals(tableNames[table])) return table;
    }
    return -1;
  }
  ...
  public String getTableName(final int table) {
    if ((table < 0) || (table >= tableCount)) throw new ArrayIndexOutOfBoundsException("Table number " + table + " doesn't exist.");
  ...
  public Table getTable(final CharSequence tableName) {
    ...
    if (table == -1) throw new ArrayIndexOutOfBoundsException("There is no table named " + tableName + ".");
  ...
  public boolean hasTable(CharSequence tableName) {
    tableName = tableName.toString();
    for (int table = 0; table < tableCount; table++) {
      if (tableName.equals(tableNames[table])) return true;
    }
    return false;
  }
