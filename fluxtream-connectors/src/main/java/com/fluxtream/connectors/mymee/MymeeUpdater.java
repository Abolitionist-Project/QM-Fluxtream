...
@Component
@Updater(prettyName = "Mymee", value = 110, updateStrategyType = Connector.UpdateStrategyType.INCREMENTAL, objectTypes = { MymeeObservationFacet.class }, extractor = MymeeObservationFacetExtractor.class)
public class MymeeUpdater extends AbstractUpdater {
	...
	final String lollipopStyle = "{\"styles\":[{\"type\":\"line\",\"show\":false,\"lineWidth\":1},"
			...
			+ "\"numberFormat\":\"###,##0\",\"type\":\"value\",\"show\":true}]," + "\"comments\":"
	...
	// TODO: This load into both the database and datastore. We should make the
	// caller no longer
	// load into the datastore.
	@Override
	protected void updateConnectorDataHistory(final UpdateInfo updateInfo) throws Exception {
		// Reset last_seq so that incremental update will pull everything
		guestService.setApiKeyAttribute(updateInfo.getGuestId(), getConnector(), "last_seq", "0");
	...
	public void updateConnectorData(UpdateInfo updateInfo) throws Exception {
		...
		long lastSeq = 0;
		try {
			lastSeq = Long.valueOf(guestService.getApiKeyAttribute(updateInfo.apiKey.getGuestId(), getConnector(),
					"last_seq"));
		} catch (Exception e) {
		}

		// Fetch and load changes, starting with lastSeq, fetching at most
		// maxToFetch each pass
		...
		while (true) {
			...
			try {
				...
				throw new Exception("Could not get Mymee observations: " + e.getMessage() + "\n" + Utils.stackTrace(e));
			}
			countSuccessfulApiCall(updateInfo.apiKey.getGuestId(), updateInfo.objectTypes, System.currentTimeMillis(),
					URL);

			// If last_seq is the same as we passed, there are no more
			// observations
			...
			for (int i = 0; i < changes.size(); i++) {
				...
				// Skip deleted objects. Someday, we might consider deleting
				// them here
				...
				if (observation.optString("type").equals("observation")) {
					...
					if (newFacet != null) {
						...
						// Channel names have all characters that aren't
						// alphanumeric or underscores replaced with underscores
			...
			// bodyTrackStorageService.storeApiData(updateInfo.apiKey.getGuestId(),
			// newFacets);

			lastSeq = newLastSeq;

			// Write lastSeq back to apiKeyAttributes
			guestService.setApiKeyAttribute(updateInfo.getGuestId(), getConnector(), "last_seq",
					String.valueOf(lastSeq));
		}

		// For each Mymee channel, setup the default display style to be
		// lollipops
	...
	// If database already has an observation with matching ID, update that
	// observation. Otherwise,
	// insert as a new observation
	// If malformed, logs an error and returns null rather than throwing an
	// exception.
	// Note that this function does not load values into the datastore

	private MymeeObservationFacet createOrUpdateObservation(final UpdateInfo updateInfo, final String rootURL,
			final JSONObject observation) {
		try {
			// mymeeId is unique for each observation
			final String mymeeId = observation.getString("_id");

			MymeeObservationFacet ret = apiDataService.createOrReadModifyWrite(MymeeObservationFacet.class,
					new FacetQuery("e.guestId = ? AND e.mymeeId = ?", updateInfo.getGuestId(), mymeeId),
					new FacetModifier<MymeeObservationFacet>() {
						// Throw exception if it turns out we can't make sense
						// of the observation's JSON
						...
						public MymeeObservationFacet createOrModify(MymeeObservationFacet facet) {
							if (facet == null) {
								...
								// auto-populate the facet's tags field with the
								// name of the observation (e.g. "Food",
								// "Back Pain", etc.)
							...
							final DateTime happened = iso8601Formatter.withZone(UTC).parseDateTime(
									observation.getString("happened"));
							...
							// Store the note in the comment field if comment
							// not already set (e.g. for photos)
							...
							try {
								// If there's an attachment, we assume there's
								// only one and that it's an image
							...
							// System.out.println("====== mymeeId=" +
							// facet.mymeeId + ", timeUpdated=" +
							// facet.timeUpdated);
							return facet;
						}
					});
	...
	// Returns root URL for mymee database, without trailing / (e.g.
	// http://hostname/databasename)
	...
	private void loadEverything(final UpdateInfo updateInfo, boolean incremental) throws Exception {
		StringBuilder sb = new StringBuilder("module=updateQueue component=updater action=loadEverythingFacet")
				.append(" connector=").append(updateInfo.apiKey.getConnector().toString()).append(" guestId=")
				.append(updateInfo.apiKey.getGuestId());
		logger.info(sb.toString());
		long then = System.currentTimeMillis();
		String queryUrl = guestService.getApiKeyAttribute(updateInfo.getGuestId(), getConnector(), "fetchURL");
		String json = null;
		try {
	...
	public Set<String> extractFacets(final String json, final UpdateInfo updateInfo, boolean incremental)
			throws Exception {
		...
		for (int i = 0; i < array.size(); i++) {
			...
			final DateTime happened = iso8601Formatter.withZone(UTC).parseDateTime(valueObject.getString("happened"));
			...
			if (incremental) {
				final List<MymeeObservationFacet> observationFacets = jpaDaoService.find("mymee.observation.byMymeeId",
						MymeeObservationFacet.class, updateInfo.getGuestId(), facet.mymeeId);
			...
			// Channel names have all characters that aren't alphanumeric or
			// underscores replaced with underscores
			channelNames.add(facet.name.replaceAll("[^0-9a-zA-Z_]+", "_"));

			// auto-populate the facet's tags field with the name of the
			// observation (e.g. "Food", "Back Pain", etc.)
			facet.addTags(Tag.cleanse(facet.name));

			if (valueObject.has("note")) {
				facet.note = valueObject.getString("note");
				facet.comment = facet.note; // also store the comment in the
											// comment field (this is required
											// for photos)
			}
			...
			try {
				if (valueObject.has("_attachments")) {
					// we assume that there's only one attachment and that it's
					// an image
					...
					if (baseURL != null && mainDir != null) {
						facet.imageURL = new StringBuilder(baseURL).append("/").append(mainDir).append("/")
								.append(facet.mymeeId).append("/").append(imageName).toString();
					}
				}
			} catch (Throwable e) {
			}
