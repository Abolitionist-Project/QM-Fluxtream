...
@Component
@Updater(prettyName = "Twitter", value = 12, updateStrategyType = com.fluxtream.connectors.Connector.UpdateStrategyType.INCREMENTAL, objectTypes = {
		TweetFacet.class, TwitterDirectMessageFacet.class, TwitterMentionFacet.class })
...
public class TwitterFeedUpdater extends AbstractUpdater {
	...
	public void getScreenName(long guestId, OAuthConsumer consumer) throws Exception {
		...
		if (response.getStatusLine().getStatusCode() == 200) {
			...
			guestService.setApiKeyAttribute(guestId, getConnector(), "screen_name", screen_name);
		}
	}
	...
	public void updateConnectorDataHistory(UpdateInfo updateInfo) throws Exception {
		...
		if (guestService.getApiKeyAttribute(updateInfo.apiKey.getGuestId(), getConnector(), "screen_name") == null)
			getScreenName(updateInfo.apiKey.getGuestId(), consumer);

		String screen_name = guestService.getApiKeyAttribute(updateInfo.apiKey.getGuestId(), getConnector(),
				"screen_name");

		List<ObjectType> objectTypes = updateInfo.objectTypes();
		if (objectTypes.contains(ObjectType.getObjectType(getConnector(), "tweet"))) {
			getStatuses(updateInfo, screen_name, consumer);
		} else if (objectTypes.contains(ObjectType.getObjectType(getConnector(), "mention"))) {
			getMentions(updateInfo, consumer);
		} else if (objectTypes.contains(ObjectType.getObjectType(getConnector(), "dm"))) {
	...
	private void refreshStatuses(UpdateInfo updateInfo, String screen_name, OAuthConsumer consumer) throws Exception {
		TweetFacet mostRecentTweet = jpaDaoService.findOne("twitter.tweet.newest", TweetFacet.class,
		...
		if (mostRecentTweet != null) {
			...
			while (newerTweets > 0) {
				...
				mostRecentTweet = jpaDaoService.findOne("twitter.tweet.newest", TweetFacet.class,
						updateInfo.apiKey.getGuestId());
			}
		}
	}
	...
	private void refreshMentions(UpdateInfo updateInfo, OAuthConsumer consumer) throws Exception {
		TwitterMentionFacet mostRecentMention = jpaDaoService.findOne("twitter.mention.newest",
				TwitterMentionFacet.class, updateInfo.apiKey.getGuestId());
		if (mostRecentMention != null) {
			...
			while (newerMentions > 0) {
				...
				mostRecentMention = jpaDaoService.findOne("twitter.mention.newest", TwitterMentionFacet.class,
						updateInfo.apiKey.getGuestId());
			}
		}
	}
	...
	private void refreshReceivedDirectMessages(UpdateInfo updateInfo, OAuthConsumer consumer) throws Exception {
		TwitterDirectMessageFacet mostRecentDM = jpaDaoService.findOne("twitter.received.dm.newest",
				TwitterDirectMessageFacet.class, updateInfo.apiKey.getGuestId());
		if (mostRecentDM != null) {
			int newerDMs = 1;
			while (newerDMs > 0) {
				newerDMs = getDirectMessagesReceivedAfter(updateInfo, mostRecentDM.twitterId + 1, consumer);
				mostRecentDM = jpaDaoService.findOne("twitter.received.dm.newest", TwitterDirectMessageFacet.class,
						updateInfo.apiKey.getGuestId());
			}
		}
	}
	...
	private void refreshSentDirectMessages(UpdateInfo updateInfo, OAuthConsumer consumer) throws Exception {
		TwitterDirectMessageFacet mostRecentDM = jpaDaoService.findOne("twitter.sent.dm.newest",
				TwitterDirectMessageFacet.class, updateInfo.apiKey.getGuestId());
		if (mostRecentDM != null) {
			int newerDMs = 1;
			while (newerDMs > 0) {
				newerDMs = getDirectMessagesSentAfter(updateInfo, mostRecentDM.twitterId + 1, consumer);
				mostRecentDM = jpaDaoService.findOne("twitter.sent.dm.newest", TwitterDirectMessageFacet.class,
						updateInfo.apiKey.getGuestId());
			}
		}
	}
	...
	private void getStatuses(UpdateInfo updateInfo, String screen_name, OAuthConsumer consumer) throws Exception {
		...
		TweetFacet oldestTweet = jpaDaoService.findOne("twitter.tweet.oldest", TweetFacet.class,
				updateInfo.apiKey.getGuestId());
		if (oldestTweet != null) {
			int olderTweets = 1;
			while (olderTweets > 0) {
				olderTweets = getStatusesBefore(updateInfo, screen_name, oldestTweet.tweetId - 1, consumer);
				oldestTweet = jpaDaoService.findOne("twitter.tweet.oldest", TweetFacet.class,
						updateInfo.apiKey.getGuestId());
			}
		}
	}
	...
	private void getMentions(UpdateInfo updateInfo, OAuthConsumer consumer) throws Exception {
		...
		TwitterMentionFacet oldestMention = jpaDaoService.findOne("twitter.mention.oldest", TwitterMentionFacet.class,
				updateInfo.apiKey.getGuestId());
		if (oldestMention != null) {
			int olderMentions = 1;
			while (olderMentions > 0) {
				olderMentions = getMentionsBefore(updateInfo, oldestMention.twitterId - 1, consumer);
				oldestMention = jpaDaoService.findOne("twitter.mention.oldest", TwitterMentionFacet.class,
						updateInfo.apiKey.getGuestId());
			}
		}
	}
	...
	private void getReceivedDirectMessages(UpdateInfo updateInfo, OAuthConsumer consumer) throws Exception {
		...
		TwitterDirectMessageFacet oldestDM = jpaDaoService.findOne("twitter.received.dm.oldest",
				TwitterDirectMessageFacet.class, updateInfo.apiKey.getGuestId());
		if (oldestDM != null) {
			int olderDMs = 1;
			while (olderDMs > 0) {
				olderDMs = getDirectMessagesReceivedBefore(updateInfo, oldestDM.twitterId - 1, consumer);
				oldestDM = jpaDaoService.findOne("twitter.received.dm.oldest", TwitterDirectMessageFacet.class,
						updateInfo.apiKey.getGuestId());
			}
		}
	}
	...
	private void getSentDirectMessages(UpdateInfo updateInfo, OAuthConsumer consumer) throws Exception {
		...
		TwitterDirectMessageFacet oldestDM = jpaDaoService.findOne("twitter.sent.dm.oldest",
				TwitterDirectMessageFacet.class, updateInfo.apiKey.getGuestId());
		if (oldestDM != null) {
			int olderDMs = 1;
			while (olderDMs > 0) {
				olderDMs = getDirectMessagesSentBefore(updateInfo, oldestDM.twitterId - 1, consumer);
				oldestDM = jpaDaoService.findOne("twitter.sent.dm.oldest", TwitterDirectMessageFacet.class,
						updateInfo.apiKey.getGuestId());
			}
		}
	}
	...
	private int getDirectMessagesReceivedBefore(UpdateInfo updateInfo, long max_id, OAuthConsumer consumer)
			throws Exception {
		return getReceivedDirectMessages(updateInfo, max_id, -1, consumer);
	}

	private int getDirectMessagesReceivedAfter(UpdateInfo updateInfo, long since_id, OAuthConsumer consumer)
			throws Exception {
		return getReceivedDirectMessages(updateInfo, -1, since_id, consumer);
	}

	private int getDirectMessagesSentBefore(UpdateInfo updateInfo, long max_id, OAuthConsumer consumer)
			throws Exception {
		return getSentDirectMessages(updateInfo, max_id, -1, consumer);
	}

	private int getDirectMessagesSentAfter(UpdateInfo updateInfo, long since_id, OAuthConsumer consumer)
			throws Exception {
		return getSentDirectMessages(updateInfo, -1, since_id, consumer);
	}

	private int getStatusesBefore(UpdateInfo updateInfo, String screen_name, long max_id, OAuthConsumer consumer)
			throws Exception {
		return getStatuses(updateInfo, screen_name, max_id, -1, consumer);
	}

	private int getStatusesAfter(UpdateInfo updateInfo, String screen_name, long since_id, OAuthConsumer consumer)
			throws Exception {
		return getStatuses(updateInfo, screen_name, -1, since_id, consumer);
	}

	private int getStatuses(UpdateInfo updateInfo, String screen_name, long max_id, long since_id,
			OAuthConsumer consumer) throws Exception {
		long then = System.currentTimeMillis();
		String requestUrl = "http://api.twitter.com/1/statuses/user_timeline.json?" + "screen_name=" + screen_name
				+ "&exclude_replies=t&count=200";
		if (max_id != -1)
	...
	private int getReceivedDirectMessages(UpdateInfo updateInfo, long max_id, long since_id, OAuthConsumer consumer)
			throws Exception {
		long then = System.currentTimeMillis();
	...
	private int getSentDirectMessages(UpdateInfo updateInfo, long max_id, long since_id, OAuthConsumer consumer)
			throws Exception {
		long then = System.currentTimeMillis();
