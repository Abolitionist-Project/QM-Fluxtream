...
public class TwitterFacetVOCollection extends AbstractFacetVOCollection {
	...
	private void addTweet(TweetFacet facet, TimeInterval timeInterval, GuestSettings settings) {
		if (tweets == null)
			tweets = new ArrayList<TweetFacetVO>();
		TweetFacetVO jsonFacet = new TweetFacetVO();
		jsonFacet.extractValues(facet, timeInterval, settings);
		tweets.add(jsonFacet);
	}

	private void addDM(TwitterDirectMessageFacet facet, TimeInterval timeInterval, GuestSettings settings) {
		if (directMessages == null)
			directMessages = new ArrayList<TwitterDirectMessageFacetVO>();
