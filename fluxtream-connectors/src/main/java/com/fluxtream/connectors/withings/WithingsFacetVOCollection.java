...
public class WithingsFacetVOCollection extends AbstractFacetVOCollection {
	...
	public void extractFacets(List facets, TimeInterval timeInterval, GuestSettings settings) {
		...
		for (Object facet : facets) {
			if (facet instanceof WithingsBodyScaleMeasureFacet)
				addWeightMeasure((WithingsBodyScaleMeasureFacet) facet, timeInterval, settings);
			else if (facet instanceof WithingsBpmMeasureFacet)
				addBpmMeasure((WithingsBpmMeasureFacet) facet, timeInterval, settings);
		}
	...
	private void sortWeightMeasures() {
		Collections.sort(weightMeasures, new Comparator<WithingsBodyScaleMeasureFacetVO>() {
			@Override
			public int compare(WithingsBodyScaleMeasureFacetVO m1, WithingsBodyScaleMeasureFacetVO m2) {
				return m1.daysAgo - m2.daysAgo;
			}
		});
	}

	private void addBpmMeasure(WithingsBpmMeasureFacet facet, TimeInterval timeInterval, GuestSettings settings) {
		if (bpmMeasures == null)
	...
	private void addWeightMeasure(WithingsBodyScaleMeasureFacet facet, TimeInterval timeInterval, GuestSettings settings) {
