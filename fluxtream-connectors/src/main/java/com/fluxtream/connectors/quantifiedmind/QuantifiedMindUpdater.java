...
@Component
@Updater(prettyName = "QuantifiedMind", value = 100, updateStrategyType = Connector.UpdateStrategyType.INCREMENTAL, objectTypes = { QuantifiedMindTestFacet.class }, extractor = QuantifiedMindTestFacetExtractor.class)
public class QuantifiedMindUpdater extends AbstractUpdater {
	...
	private void loadHistory(UpdateInfo updateInfo, long from, long to) throws Exception {
		...
		String username = guestService.getApiKeyAttribute(updateInfo.getGuestId(),
				Connector.getConnector("quantifiedmind"), "username");
		String token = guestService.getApiKeyAttribute(updateInfo.getGuestId(),
				Connector.getConnector("quantifiedmind"), "token");
		try {
			boolean partialResult = false;
			String start_time = null;
			do {
				queryUrl = "http://www.quantified-mind.com/api/get_session_data?username=" + username + "&token="
						+ token;
				if (partialResult)
			...
			countFailedApiCall(updateInfo.apiKey.getGuestId(), updateInfo.objectTypes, then, queryUrl,
					Utils.stackTrace(e));
			throw new Exception("Could not get QuantifiedMind tests: " + e.getMessage() + "\n" + Utils.stackTrace(e));
		}
