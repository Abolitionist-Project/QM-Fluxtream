...
public class DropboxController {
	...
	public String getToken(HttpServletRequest request, HttpServletResponse response) throws IOException,
		...
		String oauthCallback = env.get("homeBaseUrl") + "dropbox/upgradeToken";
		...
		OAuthProvider provider = new DefaultOAuthProvider("http://api.getdropbox.com/0/oauth/request_token",
				"http://api.getdropbox.com/0/oauth/access_token", "http://api.getdropbox.com/0/oauth/authorize");
	...
	public String upgradeToken(HttpServletRequest request, HttpServletResponse response) throws OAuthException,
		...
		guestService.setApiKeyAttribute(guest.getId(), Connector.getConnector("DROPBOX"), "accessToken",
				consumer.getToken());
		guestService.setApiKeyAttribute(guest.getId(), Connector.getConnector("DROPBOX"), "tokenSecret",
				consumer.getTokenSecret());

		return "redirect:/analyze/from/" + Connector.getConnector("DROPBOX").getName();
	}
