...
public class InstagramOAuth2Controller {
	...
	public String getToken(HttpServletRequest request, HttpServletResponse response) throws IOException,
		...
		String authorizeUrl = "https://api.instagram.com/oauth/authorize/?client_id=" + clientId + "&redirect_uri="
	...
	public String upgradeToken(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// String scope = (String)
		// request.getSession().getAttribute("oauth2Scope");
		Guest guest = AuthHelper.getGuest();

		guestService.setApiKeyAttribute(guest.getId(), Connector.getConnector("INSTAGRAM"), "accessToken",
				token.getString("access_token"));

		JSONObject userObject = token.getJSONObject("user");

		guestService.setApiKeyAttribute(guest.getId(), Connector.getConnector("INSTAGRAM"), "id",
				userObject.getString("id"));
		guestService.setApiKeyAttribute(guest.getId(), Connector.getConnector("INSTAGRAM"), "username",
				userObject.getString("username"));
		guestService.setApiKeyAttribute(guest.getId(), Connector.getConnector("INSTAGRAM"), "full_name",
				userObject.getString("full_name"));
		guestService.setApiKeyAttribute(guest.getId(), Connector.getConnector("INSTAGRAM"), "profile_picture",
				userObject.getString("profile_picture"));

		return "redirect:/analyze/from/" + Connector.getConnector("INSTAGRAM").getName();
	}
}
