...
@Component
@Updater(prettyName = "Toodledo", value = 37, objectTypes = { ToodledoTaskFacet.class, ToodledoGoalFacet.class }, updateStrategyType = UpdateStrategyType.ALWAYS_UPDATE)
...
public class ToodledoUpdater extends AbstractUpdater {
	...
	private void sync(UpdateInfo updateInfo) throws Exception {
		...
		String mostRecentJsonString = restHelper.makeRestCall(getConnector(), updateInfo.getGuestId(), -1, url);
		JSONObject mostRecentAccountInfo = JSONObject.fromObject(mostRecentJsonString);
		String lastAccountInfoJsonString = guestService.getApiKeyAttribute(updateInfo.getGuestId(), getConnector(),
				"lastAccountInfo");
		JSONObject lastSyncAccountInfo = JSONObject.fromObject(lastAccountInfoJsonString);

		if (mostRecentAccountInfo.has("errorCode"))
			throw new Exception("Could not sync with toodledo: " + mostRecentAccountInfo.getString("errorDesc"));
		if (lastAccountInfoJsonString != null) {
			if (mostRecentAccountInfo.getLong("lastedit_goal") > lastSyncAccountInfo.getLong("lastedit_goal")) {
				retrieveGoals(updateInfo, lastSyncAccountInfo.getLong("lastedit_goal"), true);
			}
		} else
			retrieveGoals(updateInfo, 0, false);
		if (lastAccountInfoJsonString != null) {
			long lastTasksUpdateLocal = Math.max(lastSyncAccountInfo.getLong("lastedit_task"),
					lastSyncAccountInfo.getLong("lastdelete_task"));
			long lastTasksUpdateRemote = Math.max(mostRecentAccountInfo.getLong("lastedit_task"),
		...
		guestService.setApiKeyAttribute(updateInfo.getGuestId(), getConnector(), "lastAccountInfo",
				mostRecentJsonString);
	}

	private void retrieveGoals(UpdateInfo updateInfo, long since, boolean update) throws RateLimitReachedException,
			Exception {
		String key = getKey(updateInfo);
		String urlString = TOODLEDO_GOALS_GET + "?key=" + key;
		String goalsJson = restHelper.makeRestCall(getConnector(), updateInfo.getGuestId(), 2, urlString);
		apiDataService.eraseApiData(updateInfo.getGuestId(), getConnector(), 2);
	...
	private void retrieveTasks(UpdateInfo updateInfo, long since, boolean update) throws RateLimitReachedException,
			Exception {
		String key = getKey(updateInfo);
		String urlString = TOODLEDO_TASKS_GET + "?key=" + key + "&after=" + since;
		String tasksJson = restHelper.makeRestCall(getConnector(), updateInfo.getGuestId(), 1, urlString);
		...
		for (int i = 0; i < arrayInfo.getInt("total"); i++) {
			...
			if (update) {
				ToodledoTaskFacet oldTask = jpaDaoService.findOne("toodledo.task.byToodledoId",
						ToodledoTaskFacet.class, updateInfo.getGuestId(), toodledo_id);
				if (oldTask != null)
			...
			taskFacet.api = getConnector().value();
			taskFacet.objectType = 1;
			jpaDaoService.persist(taskFacet);
		}
		if (update) {
			urlString = TOODLEDO_TASKS_GET_DELETED + "?key=" + key + "&after=" + since;
			String tasksToDeleteJson = restHelper.makeRestCall(getConnector(), updateInfo.getGuestId(), 1, urlString);
			...
			for (int i = 0; i < arrayInfo.getInt("num"); i++) {
				...
				long toodledo_id = task.getLong("id");
				ToodledoTaskFacet taskToDelete = jpaDaoService.findOne("toodledo.task.byToodledoId",
						ToodledoTaskFacet.class, updateInfo.getGuestId(), toodledo_id);
	...
	private String getKey(UpdateInfo updateInfo) throws Exception {
		...
		String userPassword = guestService.getApiKeyAttribute(updateInfo.getGuestId(), getConnector(), "password");
		return hash(hash(userPassword) + env.get("toodledo.appToken") + sessionToken);
	}

	private String getSessionToken(UpdateInfo updateInfo) throws Exception {
		String token = "";
		String sessionTokenExpires = guestService.getApiKeyAttribute(updateInfo.getGuestId(), getConnector(),
				"sessionToken-expires");
		long now = System.currentTimeMillis();
		if (sessionTokenExpires == null || Long.valueOf(sessionTokenExpires) < now) {
			long elapsed = now - Long.valueOf(sessionTokenExpires);
			System.out.println("token has expired since " + elapsed + " ms");
			String userid = guestService.getApiKeyAttribute(updateInfo.getGuestId(), getConnector(), "userid");
			token = getSessionToken(updateInfo, userid);
			guestService.setApiKeyAttribute(updateInfo.getGuestId(), getConnector(), "sessionToken", token);
			String fourHoursFromNow = String.valueOf(now + (3600000 * 4));
			guestService.setApiKeyAttribute(updateInfo.getGuestId(), getConnector(), "sessionToken-expires",
					fourHoursFromNow);
		} else
			token = guestService.getApiKeyAttribute(updateInfo.getGuestId(), getConnector(), "sessionToken");
		return token;
	}
	...
	public String getSessionToken(UpdateInfo updateInfo, String userid) throws RateLimitReachedException, Exception {
		String appId = env.get("toodledo.appId");
		String appToken = env.get("toodledo.appToken");
		String signature = hash(userid + appToken);
		String url = TOODLEDO_ACCOUNT_TOKEN + "?userid=" + userid + ";appid=" + appId + ";sig=" + signature;
		try {
			String json = restHelper.makeRestCall(getConnector(), updateInfo.getGuestId(), -1, url);
	...
	public String getToodledoUserid(long guestId, String email, String password) throws RateLimitReachedException,
			Exception {
		...
		String url = TOODLEDO_ACCOUNT_LOOKUP + "?appid=" + appId + ";sig=" + signature + ";email=" + email + ";pass="
				+ password;
		String json = restHelper.makeRestCall(getConnector(), guestId, -1, url);
