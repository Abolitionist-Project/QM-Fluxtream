...
public class FlickrUpdater extends AbstractUpdater {
	...
	protected void updateConnectorDataHistory(UpdateInfo updateInfo) throws RateLimitReachedException, Exception {
		...
		if (!connectorUpdateService.isHistoryUpdateCompleted(updateInfo.getGuestId(), getConnector().getName(), -1))
			apiDataService.eraseApiData(updateInfo.getGuestId(), getConnector(), -1);
	...
	public void updateConnectorData(UpdateInfo updateInfo) throws Exception {
		...
		ApiUpdate lastSuccessfulUpdate = connectorUpdateService.getLastSuccessfulUpdate(updateInfo.apiKey.getGuestId(),
				getConnector());
		for (int page = 0; retrievedItems == ITEMS_PER_PAGE; page++) {
			JSONObject feed = retrievePhotoHistory(updateInfo, lastSuccessfulUpdate.ts, System.currentTimeMillis(),
					page);
			JSONObject photosWrapper = feed.getJSONObject("photos");
	...
	private JSONObject retrievePhotoHistory(UpdateInfo updateInfo, long from, long to, int page) throws Exception {
		long then = System.currentTimeMillis();

		String api_key = env.get("flickrConsumerKey");
		String nsid = guestService.getApiKeyAttribute(updateInfo.apiKey.getGuestId(), getConnector(), "nsid");
		String token = guestService.getApiKeyAttribute(updateInfo.apiKey.getGuestId(), getConnector(), "token");

		// The start/end upload dates should be in the form of a unix timestamp
		// (see http://www.flickr.com/services/api/flickr.people.getPhotos.htm)
		...
		String searchPhotosUrl = "http://api.flickr.com/services/rest/" + "?method=flickr.people.getPhotos&api_key="
				+ api_key + "&per_page=" + ITEMS_PER_PAGE + "&page=" + page + "&api_key=" + api_key + "&user_id="
				+ nsid + "&auth_token=" + token
				+ "&format=json&nojsoncallback=1&extras=date_upload,date_taken,description,geo,tags"
				+ "&min_upload_date=" + startDate + "&max_upload_date=" + endDate + "&api_sig=" + api_sig;
		searchPhotosUrl = searchPhotosUrl.replace(" ", "%20");
		String photosJson = null;
		try {
			photosJson = fetch(searchPhotosUrl);
			countSuccessfulApiCall(updateInfo.getGuestId(), updateInfo.objectTypes, then, searchPhotosUrl);
		} catch (Exception e) {
			countFailedApiCall(updateInfo.getGuestId(), updateInfo.objectTypes, then, searchPhotosUrl,
					Utils.stackTrace(e));
			throw e;
		}
