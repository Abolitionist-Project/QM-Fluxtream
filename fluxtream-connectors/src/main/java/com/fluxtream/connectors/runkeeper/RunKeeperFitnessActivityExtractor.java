	...
	public List<AbstractFacet> extractFacets(final ApiData apiData, final ObjectType objectType) throws Exception {
		...
		if (jsonObject.has("path")) {
			...
			for (int i = 0; i < path.size(); i++) {
				...
				// locationFacet.latitude = (float)
				// pathElement.getDouble("latitude");
				// locationFacet.longitude = (float)
				// pathElement.getDouble("longitude");
				// if (!startTimeSet) {
				// // we need to know the user's location in order to figure out
				// // his timezone
				// final String start_time = jsonObject.getString("start_time");
				// final TimeZone timeZone =
				// metadataService.getTimeZone(locationFacet.latitude,
				// locationFacet.longitude);
				// facet.start =
				// timeFormatter.withZone(DateTimeZone.forTimeZone(timeZone)).parseMillis(start_time);
				...
				// startTimeSet = true;
				// }
				// locationFacet.altitude = (int)
				// pathElement.getDouble("altitude");
				// locationFacet.timestampMs = facet.start +
				// pathElement.getLong("timestamp");
			...
			// TODO: abort elegantly if we don't have gps data as we are unable
			// to figure out time
			// in this case
			return facets;
		}
