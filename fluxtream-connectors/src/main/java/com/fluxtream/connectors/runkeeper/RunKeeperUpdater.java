 ...
 */
@Component
@Updater(prettyName = "RunKeeper", value = 35, updateStrategyType = Connector.UpdateStrategyType.INCREMENTAL, objectTypes = { RunKeeperFitnessActivityFacet.class })
public class RunKeeperUpdater extends AbstractUpdater {
	...
	protected void updateConnectorDataHistory(final UpdateInfo updateInfo) throws Exception {
		...
		guestService.setApiKeyAttribute(updateInfo.getGuestId(), updateInfo.apiKey.getConnector(), "lastUpdated",
	...
	private void getFitnessActivities(final UpdateInfo updateInfo, final OAuthService service, final Token token,
			final List<String> activities) throws Exception {
	...
	final DateTimeFormatter dateFormatter = DateTimeFormat.forPattern("EEE, dd MMM yyyy HH:mm:ss Z").withZone(
			DateTimeZone.forID("GMT"));

	private void getFitnessActivityFeed(final OAuthService service, final Token token, String activityFeedURL,
			final int pageSize, List<String> activities, long since) {
