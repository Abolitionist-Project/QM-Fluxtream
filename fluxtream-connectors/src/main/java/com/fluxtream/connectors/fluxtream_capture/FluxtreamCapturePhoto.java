...
public final class FluxtreamCapturePhoto {
	...
	private static final String CONNECTOR_PRETTY_NAME = Connector.getConnector(FluxtreamCaptureUpdater.CONNECTOR_NAME)
			.prettyName();
	private static final String OBJECT_TYPE_NAME = "photo";

	@NotNull
	public static String createPhotoStoreKey(final long guestId, @NotNull final String captureYYYYDDD,
			final long captureTimeMillisUtc, @NotNull final String photoHash) {
		return guestId + KEY_VALUE_STORE_KEY_PART_DELIMITER + CONNECTOR_PRETTY_NAME
				+ KEY_VALUE_STORE_KEY_PART_DELIMITER + OBJECT_TYPE_NAME + KEY_VALUE_STORE_KEY_PART_DELIMITER
				+ captureYYYYDDD + KEY_VALUE_STORE_KEY_PART_DELIMITER + captureTimeMillisUtc
				+ KEY_VALUE_STORE_FILENAME_PART_DELIMITER + photoHash;
	}
	...
	FluxtreamCapturePhoto(final long guestId, @NotNull final byte[] photoBytes, final long captureTimeMillisUtc)
			throws IllegalArgumentException, NoSuchAlgorithmException, IOException,
			FluxtreamCapturePhotoStore.UnsupportedImageFormatException {
		...
		if (tempImageType == null) {
			throw new FluxtreamCapturePhotoStore.UnsupportedImageFormatException(
					"The photoBytes do not contain a supported image format");
		}
		...
		final String dayOfYear = String.format("%03d", captureTime.getDayOfYear()); // pad
																					// with
																					// zeros
																					// so
																					// that
		...
		// Create the thumbnails: do so by creating the largest one first, and
		// then creating the smaller
		// ones from the larger--this should be faster than creating each from
		// the original image
		...
		final Image thumbnail1Image = ImageUtils.createJpegThumbnail(thumbnail2Image.getBytes(),
				THUMBNAIL_1_MAX_SIDE_LENGTH_IN_PIXELS);
		if (thumbnail1Image == null) {
			throw new IOException("Failed to create thumbnails");
		}

		final Image thumbnail0Image = ImageUtils.createJpegThumbnail(thumbnail1Image.getBytes(),
				THUMBNAIL_0_MAX_SIDE_LENGTH_IN_PIXELS);
		...
		// get the image orientation, and default to ORIENTATION_1 if
		// unspecified
		ImageOrientation orientationTemp;
		try {
			orientationTemp = ImageUtils.getOrientation(photoBytes);
		} catch (Exception e) {
			LOG.error("Exception while trying to read the orientation data for user [" + guestId + "] photo ["
					+ photoStoreKey + "]");
		...
		try {
			...
			LOG.error("ImageProcessingException while trying to read the geolocation data for user [" + guestId
					+ "] photo [" + photoStoreKey + "]");
