...
/**
 * <p>
 * <code>FluxtreamCapturePhotoStore</code> enables managment of Fluxtream
 * Capture photos.
 * </p>
 * 
...
public final class FluxtreamCapturePhotoStore {
	...
	public interface Photo {
		...
		/**
		 * A {@link String} representation of the unique identifier for this
		 * photo. Useful for logging and messages.
		 */
		@NotNull
	...
	/**
	 * Returns the photo specified by the given <code>photoStoreKey</code> or
	 * <code>null</code> if no such photo exists. This method assumes that the
	 * caller has already performed authentication and authorization.
	 */
	@Nullable
			...
			if (bytes != null) {
				return new Photo() {
					...
					public ImageType getImageType() {
						// Try to read the image type. If we can't for some
						// reason, then just lie and say it's a JPEG.
						// This really should never happen, but it's good to
						// check for it anyway and log a warning if it
						// happens.
						ImageType imageType = ImageUtils.getImageType(bytes);
						if (imageType == null) {
							imageType = ImageType.JPEG;
							LOG.warn("FluxtreamCapturePhotoStore.getImageType(): Could not determine the media type for photo ["
									+ getIdentifier() + "]!  Defaulting to [" + imageType.getMediaType() + "]");
						}
	...
	/**
	 * Returns the photo thumbnail specified by the given <code>photoId</code>
	 * or <code>null</code> if no such photo exists. This method assumes that
	 * the caller has already performed authentication and authorization.
	 */
	@Nullable
	public Photo getPhotoThumbnail(final long uid, final long photoId, final int thumbnailIndex) {
		final FluxtreamCapturePhotoFacet photoFacet = jpaDaoService.findOne("fluxtream_capture.photo.byId",
				FluxtreamCapturePhotoFacet.class, uid, photoId);
	...
	/**
	 * Saves the given photo for the given user to both the database and the
	 * Fluxtream Capture key-value photo store. Returns an
	 * {@link OperationResult} if the photo was created or updated (see the
	 * {@link OperationResult#getOperation()} method to determine which
	 * occurred), or throws a {@link FluxtreamCapturePhotoStoreException}
	 * exception if the save/update failed.
	 * 
	 * @throws InvalidDataException
	 *             if the photo or its metadata is <code>null</code>, empty, or
	 *             in some way invalid
	 * @throws StorageException
	 *             if the save/update fails to either the photo key-value store
	 *             or the database
	 * @throws UnsupportedImageFormatException
	 *             if the save/update fails because the given image is not of a
	 *             supported format
	 */
	@SuppressWarnings("ConstantConditions")
	public OperationResult<FluxtreamCapturePhoto> saveOrUpdatePhoto(final long guestId,
			@NotNull final byte[] photoBytes, @NotNull final String jsonMetadata) throws StorageException,
			InvalidDataException, UnsupportedImageFormatException {
		if (LOG_DEBUG.isDebugEnabled()) {
			LOG_DEBUG.debug("FluxtreamCapturePhotoStore.savePhoto(" + guestId + ", " + photoBytes.length + ", "
					+ jsonMetadata + ")");
		}
		...
		// Go ahead and try to create the FilesystemKeyValueStore. This is a
		// simple operation, so, if it's going
		// to fail, then it's better to fail now rather than after spending a
		// lot of effort creating the photo hash
		// and thumbnails.
		...
		// Now that we know the metadata is valid, we convert the capture time
		// from seconds to milliseconds
		final long captureTimeMillisUtc = (long) (metadata.capture_time_secs_utc * 1000);

		// Create the FluxtreamCapturePhoto (this validates the photo, generates
		// the hash and thumbnails, etc.)
		...
		// Now that we have the key-value store created and everything appears
		// to be valid, we can go ahead and
		// insert the photo into the photo key-value store, but only if the key
		...
		// The photo is in the key-value store, so try to save or update to the
		// DB
		...
		try {
			photoFacet = apiDataService.createOrReadModifyWrite(FluxtreamCapturePhotoFacet.class,
					photoCreatorOrModifier.getFacetFinderQuery(), photoCreatorOrModifier);
		} catch (Exception e) {
		...
		// If we got this far, then we know everything succeeded, so simply
		// return the boolean to indicate whether
		// the photo was created or updated
		if (LOG_DEBUG.isInfoEnabled() || LOG.isInfoEnabled()) {
			final String message = "FluxtreamCapturePhotoStore.saveOrUpdatePhoto(): photo [" + photoFacet.getHash()
					+ "] " + (photoCreatorOrModifier.wasCreated() ? "saved" : "updated") + " sucessfully for user ["
					+ guestId + "]";
			LOG.info(message);
	...
	private static final class PhotoCreatorOrModifier implements
			ApiDataService.FacetModifier<FluxtreamCapturePhotoFacet> {
		@NotNull
		...
		public PhotoCreatorOrModifier(@NotNull final FluxtreamCapturePhoto photo) {
			this.photo = photo;
			facetFinderQuery = new ApiDataService.FacetQuery("e.guestId = ? AND e.hash = ? AND e.start = ?",
					photo.getGuestId(), photo.getPhotoHash(), photo.getCaptureTimeMillisUtc());
		}
		...
		public FluxtreamCapturePhotoFacet createOrModify(final FluxtreamCapturePhotoFacet existingFacet) {
			...
			if (existingFacet == null) {
				...
				// We already have this photo, so we don't need to do anything
				// other than update the timeUpdated field
