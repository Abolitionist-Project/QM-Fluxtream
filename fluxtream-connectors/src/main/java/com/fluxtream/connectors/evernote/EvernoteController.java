...
public class EvernoteController {
	...
	public String getEvernoteToken(HttpServletRequest request) throws IOException, ServletException {
		OAuthService service = new ServiceBuilder().provider(EvernoteApi.Sandbox.class).apiKey(getConsumerKey())
				.apiSecret(getConsumerSecret()).callback(env.get("homeBaseUrl") + "evernote/upgradeToken").build();
	...
	public String upgradeToken(HttpServletRequest request) throws IOException {
		...
		return "redirect:/analyze/from/evernote";
	}
