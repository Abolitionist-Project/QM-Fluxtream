...
public class LatitudeSample {
	...
	// LatitudeSample.class.getResourceAsStream("/client_secrets.json"));
	// if (clientSecrets.getDetails().getClientId().startsWith("Enter")
	// || clientSecrets.getDetails().getClientSecret().startsWith("Enter ")) {
	// System.out.println("Enter Client ID and Secret from https://code.google.com/apis/console/?api=latitude "
	// + "into latitude-cmdline-sample/src/main/resources/client_secrets.json");
	// System.exit(1);
	// }
	// // set up file credential store
	// FileCredentialStore credentialStore = new FileCredentialStore(new
	// File(System.getProperty("user.home"),
	// ".credentials/latitude.json"), JSON_FACTORY);
	// // set up authorization code flow
	// GoogleAuthorizationCodeFlow flow = new
	// GoogleAuthorizationCodeFlow.Builder(HTTP_TRANSPORT, JSON_FACTORY,
	// clientSecrets,
	// Collections.singleton(LatitudeScopes.LATITUDE_ALL_BEST)).setCredentialStore(
	...
	//
	// // set up Latitude
	// Latitude latitude = new Latitude.Builder(HTTP_TRANSPORT, JSON_FACTORY,
	// credential)
	...
	// }
	//
	// private static void showCurrentLocation(Latitude latitude) throws
	// IOException {
	...
	// }
	//
	// private static void showLocationHistory(Latitude latitude) throws
	// IOException {
