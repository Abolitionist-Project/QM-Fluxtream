...
@Component
@Controller
@Updater(prettyName = "Fitbit", value = 7, objectTypes = { FitbitTrackerActivityFacet.class,
		FitbitLoggedActivityFacet.class, FitbitSleepFacet.class, FitbitWeightFacet.class }, defaultChannels = {
		"Fitbit.steps", "Fitbit.caloriesOut" })
...
public class FitBitTSUpdater extends AbstractUpdater implements Autonomous {
	...
	final ObjectType sleepOT = ObjectType.getObjectType(getConnector(), "sleep");
	final ObjectType weightOT = ObjectType.getObjectType(getConnector(), "weight");
	final ObjectType activityOT = ObjectType.getObjectType(getConnector(), "activity_summary");
	final ObjectType loggedActivityOT = ObjectType.getObjectType(getConnector(), "logged_activity");
	...
	public void updateConnectorDataHistory(UpdateInfo updateInfo) throws Exception {
		...
		loadTimeSeries("sleep/timeInBed", updateInfo.apiKey, sleepOT, "timeInBed");
		loadTimeSeries("sleep/startTime", updateInfo.apiKey, sleepOT, "startTime");
		loadTimeSeries("sleep/minutesAsleep", updateInfo.apiKey, sleepOT, "minutesAsleep");
		loadTimeSeries("sleep/minutesAwake", updateInfo.apiKey, sleepOT, "minutesAwake");
		loadTimeSeries("sleep/minutesToFallAsleep", updateInfo.apiKey, sleepOT, "minutesToFallAsleep");
		loadTimeSeries("sleep/minutesAfterWakeup", updateInfo.apiKey, sleepOT, "minutesAfterWakeup");
		loadTimeSeries("sleep/awakeningsCount", updateInfo.apiKey, sleepOT, "awakeningsCount");

		// activities

		loadTimeSeries("activities/tracker/calories", updateInfo.apiKey, activityOT, "caloriesOut");
		loadTimeSeries("activities/tracker/steps", updateInfo.apiKey, activityOT, "steps");
		loadTimeSeries("activities/tracker/distance", updateInfo.apiKey, activityOT, "totalDistance");
		...
		loadTimeSeries("activities/tracker/minutesSedentary", updateInfo.apiKey, activityOT, "sedentaryMinutes");
		loadTimeSeries("activities/tracker/minutesLightlyActive", updateInfo.apiKey, activityOT, "lightlyActiveMinutes");
		loadTimeSeries("activities/tracker/minutesFairlyActive", updateInfo.apiKey, activityOT, "fairlyActiveMinutes");
		loadTimeSeries("activities/tracker/minutesVeryActive", updateInfo.apiKey, activityOT, "veryActiveMinutes");
		loadTimeSeries("activities/tracker/activeScore", updateInfo.apiKey, activityOT, "activeScore");
		loadTimeSeries("activities/tracker/activityCalories", updateInfo.apiKey, activityOT, "activityCalories");

		// weight

		loadTimeSeries("body/weight", updateInfo.apiKey, weightOT, "weight");
		loadTimeSeries("body/bmi", updateInfo.apiKey, weightOT, "bmi");
		loadTimeSeries("body/fat", updateInfo.apiKey, weightOT, "fat");

		jpaDaoService
				.execute("DELETE FROM Facet_FitbitSleep sleep WHERE sleep.startTimeStorage=null and sleep.endTimeStorage=null");
		...
		guestService.setApiKeyAttribute(updateInfo.getGuestId(), getConnector(), "TRACKER.lastSyncDate",
				String.valueOf(trackerLastSyncDate));
		guestService.setApiKeyAttribute(updateInfo.getGuestId(), getConnector(), "SCALE.lastSyncDate",
	...
	public void updateCaloriesIntraday(FitbitTrackerActivityFacet facet, ApiKey apiKey)
		...
		if (facet.date != null) {
			if (facet.caloriesJson == null || isToday(facet.date, apiKey.getGuestId())) {
				String json = signpostHelper.makeRestCall(getConnector(), apiKey,
						"activities/log/calories/date".hashCode(),
						"http://api.fitbit.com/1/user/-/activities/log/calories/date/" + facet.date + "/1d.json");
	...
	private boolean isToday(String date, long guestId) {
		...
		String today = dateFormat.withZone(DateTimeZone.forTimeZone(tz)).print(System.currentTimeMillis());
		return date.equals(today);
	}

	public void updateStepsIntraday(FitbitTrackerActivityFacet facet, ApiKey apiKey) throws RateLimitReachedException {
		if (facet.date != null) {
			String json = signpostHelper.makeRestCall(getConnector(), apiKey, "activities/log/steps/date".hashCode(),
					"http://api.fitbit.com/1/user/-/activities/log/steps/date/" + facet.date + "/1d.json");
	...
	public void loadTimeSeries(String uri, ApiKey apiKey, ObjectType objectType, String fieldName)
			throws RateLimitReachedException {

		String json = signpostHelper.makeRestCall(getConnector(), apiKey, uri.hashCode(),
				"http://api.fitbit.com/1/user/-/" + uri + "/date/today/max.json");
		...
		for (int i = 0; i < timeSeriesArray.size(); i++) {
			try {
				...
				if (objectType == sleepOT) {
					FitbitSleepFacet facet = getSleepFacet(apiKey.getGuestId(), date);
					if (facet == null) {
						facet = new FitbitSleepFacet();
						facet.date = date;
						facet.api = getConnector().value();
					...
					FitbitTrackerActivityFacet facet = getActivityFacet(apiKey.getGuestId(), timeInterval);
					if (facet == null) {
	...
	private FitbitWeightFacet getWeightFacet(final long guestId, final TimeInterval timeInterval) {
		return jpaDaoService.findOne("fitbit.weight.byStartEnd", FitbitWeightFacet.class, guestId, timeInterval.start,
				timeInterval.end);
	}

	private FitbitTrackerActivityFacet getActivityFacet(long guestId, TimeInterval timeInterval) {
		return jpaDaoService.findOne("fitbit.activity_summary.byStartEnd", FitbitTrackerActivityFacet.class, guestId,
				timeInterval.start, timeInterval.end);
	}

	private FitbitSleepFacet getSleepFacet(long guestId, String date) {
		return jpaDaoService.findOne("fitbit.sleep.byDate", FitbitSleepFacet.class, guestId, date);
	}

	@Transactional(readOnly = false)
	private void addToSleepFacet(FitbitSleepFacet facet, JSONObject entry, String fieldName, DayMetadataFacet md) {
	...
	private final static DateTimeFormatter format = DateTimeFormat.forPattern("yyyy-MM-dd'T'HH:mm:ss.SSS");

	private void storeTime(String bedTimeString, FitbitSleepFacet facet, DayMetadataFacet md) {
		...
		bedTimeUTC = format.withZone(DateTimeZone.forID("UTC")).parseMutableDateTime(bedTimeString);
		bedTimeUTC.add(facet.timeInBed * 60000);
		String riseTimeString = format.withZone(DateTimeZone.forID("UTC")).print(bedTimeUTC.getMillis());
		facet.start = format.withZone(DateTimeZone.forID(md.timeZone)).parseDateTime(bedTimeString).getMillis();
		facet.end = format.withZone(DateTimeZone.forID(md.timeZone)).parseDateTime(riseTimeString).getMillis();
		facet.endTimeStorage = riseTimeString;
	}

	@Transactional(readOnly = false)
	private void addToActivityFacet(FitbitTrackerActivityFacet facet, JSONObject entry, String fieldName) {
		setFieldValue(facet, fieldName, entry.getString("value"));
		facetDao.merge(facet);
	}
	...
	public List<String> getDaysSinceLastSync(ApiKey apiKey, final String deviceType, long latestTrackerSyncDate,
			long latestScaleSyncDate) throws RateLimitReachedException {
		long lastSyncDate = 0;
		if (deviceType.equals("TRACKER")) {
			final String trackerLastSyncDate = guestService.getApiKeyAttribute(apiKey.getGuestId(), getConnector(),
					"TRACKER.lastSyncDate");
			lastSyncDate = Long.valueOf(trackerLastSyncDate);
		} else {
			final String scaleLastSyncDate = guestService.getApiKeyAttribute(apiKey.getGuestId(), getConnector(),
					"SCALE.lastSyncDate");
	...
	private JSONArray getDeviceStatusesArray(final ApiKey apiKey) throws RateLimitReachedException {
		...
		String json = signpostHelper.makeRestCall(getConnector(), apiKey, getUserDevicesObjectTypeID, urlString);
	...
	public void updateConnectorData(UpdateInfo updateInfo) throws Exception {
		if (updateInfo.jsonParams != null) {
			...
			Date date = new Date(dateFormat.withZone(DateTimeZone.forTimeZone(timeZone)).parseMillis(dateString));
			...
			if (trackerLastSyncDate > -1) {
				final List<String> trackerDaysToSync = getDaysSinceLastSync(updateInfo.apiKey, "TRACKER",
						trackerLastSyncDate, scaleLastSyncDate);
	...
	private void updateOneDayOfData(final UpdateInfo updateInfo, final List<ObjectType> objectTypes,
			final TimeZone userTimeZone, final Date date, final String dateString) throws Exception {
		...
		TimeInterval timeInterval = new TimeInterval(from, to, TimeUnit.DAY, userTimeZone);
		...
		if (objectTypes.contains(sleepOT)) {
			...
			apiDataService.eraseApiData(updateInfo.getGuestId(), updateInfo.apiKey.getConnector(), sleepOT,
		...
		if (objectTypes.contains(activityOT)) {
			logger.info("guestId=" + updateInfo.getGuestId() + " objectType=activity"
					+ " connector=fitbit objectType=weightMeasurement action=updateOneDayOfData date=" + dateString);
			apiDataService.eraseApiData(updateInfo.getGuestId(), updateInfo.apiKey.getConnector(), activityOT,
					Arrays.asList(dateString));
			apiDataService.eraseApiData(updateInfo.getGuestId(), updateInfo.apiKey.getConnector(), loggedActivityOT,
					Arrays.asList(dateString));
			try {
		...
		if (objectTypes.contains(weightOT)) {
			logger.info("guestId=" + updateInfo.getGuestId() + " objectType=weight"
					+ " connector=fitbit objectType=weightMeasurement action=updateOneDayOfData date=" + dateString);
			apiDataService.eraseApiData(updateInfo.getGuestId(), updateInfo.apiKey.getConnector(), weightOT,
					Arrays.asList(dateString));
			try {
	...
	private void loadWeightDataForOneDay(UpdateInfo updateInfo, Date date, TimeZone timeZone, String formattedDate)
		...
		logger.info("guestId=" + updateInfo.getGuestId() + " connector=fitbit action=loadWeightDataForOneDay json="
				+ json);
		if (json != null) {
	...
	private void loadActivityDataForOneDay(UpdateInfo updateInfo, Date date, TimeZone timeZone, String formattedDate)
			throws RateLimitReachedException, Exception {
		String json = getActivityData(updateInfo, formattedDate);
		long fromMidnight = TimeUtils.fromMidnight(date.getTime(), timeZone);
		long toMidnight = TimeUtils.toMidnight(date.getTime(), timeZone);
		logger.info("guestId=" + updateInfo.getGuestId() + " connector=fitbit action=loadActivityDataForOneDay json="
				+ json);
		if (json != null) {
			apiDataService.cacheApiDataJSON(updateInfo, json, fromMidnight, toMidnight, activityOT.value()
					+ loggedActivityOT.value());
		}
	}

	private void loadSleepDataForOneDay(UpdateInfo updateInfo, Date date, TimeZone timeZone, String formattedDate)
			throws RateLimitReachedException, Exception {
		String json = getSleepData(updateInfo, formattedDate);
		long fromMidnight = TimeUtils.fromMidnight(date.getTime(), timeZone);
		long toMidnight = TimeUtils.toMidnight(date.getTime(), timeZone);
		if (json != null) {
			apiDataService.cacheApiDataJSON(updateInfo, json, fromMidnight, toMidnight, sleepOT.value());
		}
	}

	private String getSleepData(UpdateInfo updateInfo, String formattedDate) throws RateLimitReachedException {
		String urlString = "http://api.fitbit.com/1/user/-/sleep/date/" + formattedDate + ".json";

		String json = signpostHelper.makeRestCall(getConnector(), updateInfo.apiKey, sleepOT.value(), urlString);

		return json;
	}

	private String getWeightData(UpdateInfo updateInfo, String formattedDate) throws RateLimitReachedException {
		String urlString = "http://api.fitbit.com/1/user/-/body/log/weight/date/" + formattedDate + ".json";

		String json = signpostHelper.makeRestCall(getConnector(), updateInfo.apiKey, weightOT.value(), urlString);
	...
	private String getActivityData(UpdateInfo updateInfo, String formattedDate) throws RateLimitReachedException {
		...
		String urlString = "http://api.fitbit.com/1/user/-/activities/date/" + formattedDate + ".json";

		String json = signpostHelper.makeRestCall(getConnector(), updateInfo.apiKey, activityOT.value()
				+ loggedActivityOT.value(), urlString);

		return json;
	}

	@RequestMapping("/fitbit/notify")
	public void notifyMeasurement(@RequestBody String updatesString, HttpServletRequest request,
		...
		logger.info("action=apiNotification connector=fitbit message=" + updatesString);
		...
		try {
			...
			for (int i = 0; i < updatesArray.size(); i++) {
				...
				FitbitUserProfile userProfile = jpaDaoService.findOne("fitbitUser.byEncodedId",
						FitbitUserProfile.class, ownerId);

				long guestId = userProfile.guestId;

				int objectTypes = 0;
				if (collectionType.equals("foods") || collectionType.equals("body")) {
					// notificationsService.addNotification(guestId,
					// Notification.Type.INFO,
					// "Received new body info from Fitbit");
					continue;
				} else if (collectionType.equals("activities")) {
					// notificationsService.addNotification(guestId,
					// Notification.Type.INFO,
					// "Received new activity info from Fitbit");
					objectTypes = 3;
				} else if (collectionType.equals("sleep")) {
					// notificationsService.addNotification(guestId,
					// Notification.Type.INFO,
					// "Received new sleep info from Fitbit");
					objectTypes = 4;
				}

				connectorUpdateService.addApiNotification(getConnector(), userProfile.guestId, updatesString);

				JSONObject jsonParams = new JSONObject();
				jsonParams.accumulate("date", dateString).accumulate("ownerId", ownerId)
						.accumulate("subscriptionId", subscriptionId);

				logger.info("action=scheduleUpdate connector=fitbit collectionType=" + collectionType);

				connectorUpdateService.scheduleUpdate(userProfile.guestId, getConnector().getName(), objectTypes,
						UpdateType.PUSH_TRIGGERED_UPDATE, System.currentTimeMillis() + 5000, jsonParams.toString());
			}
		} catch (Exception e) {
			System.out.println("error processing fitbit notification " + updatesString);
			e.printStackTrace();
			logger.warn("Could not parse fitbit notification: " + Utils.stackTrace(e));
		}
	}
