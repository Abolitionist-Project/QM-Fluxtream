...
public class FacebookConnectorController {
	...
	public String getToken(HttpServletRequest request, HttpServletResponse response) throws IOException,
		...
		String authorizeUrl = "https://www.facebook.com/dialog/oauth?" + "redirect_uri=" + redirectUri + "&client_id="
				+ clientId + "&scope=read_stream,user_activities,user_birthday,"
				+ "user_checkins,user_events,user_location,user_notes,"
				+ "user_online_presence,user_photos,user_relationships," + "user_videos,user_work_history";

		return "redirect:" + authorizeUrl;
	}

	@RequestMapping(value = "/swapToken")
	public ModelAndView upgradeToken(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		String fetched = "";
		try {
			String swapTokenUrl = "https://graph.facebook.com/oauth/access_token?" + "client_id="
					+ params.get("client_id") + "&redirect_uri=" + redirectUri + "&client_secret="
					+ params.get("client_secret") + "&code=" + code;
		...
		if (!hasError) {
			...
			if (!access_token.equals("")) {
				...
				guestService.setApiKeyAttribute(guest.getId(), connector, "accessToken", access_token);
				guestService.setApiKeyAttribute(guest.getId(), connector, "expires", expires);
