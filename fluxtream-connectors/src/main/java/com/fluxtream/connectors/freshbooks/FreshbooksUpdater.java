...
public class FreshbooksUpdater extends AbstractUpdater {
	...
	public void updateConnectorData(UpdateInfo updateInfo) throws Exception {
		...
		OAuthConsumer consumer = new DefaultOAuthConsumer(env.get("freshbooksConsumerKey"),
		...
		Writer requestWriter = new OutputStreamWriter(request.getOutputStream());

		requestWriter.write("<!--?xml version=\"1.0\" encoding=\"utf-8\"?-->" + "<request method=\"project.list\">  "
				+ "<!-- The page number to show (Optional) -->  " + "<page>1</page>  "
				+ "<!-- Number of results per page, default 25 (Optional) -->  " + "<per_page>15</per_page>  "
				+ "</request>  ");
		...
		if (request.getResponseCode() == 200) {
			...
			apiDataService.cacheApiDataXML(updateInfo, xml, -1, -1);
		} else {
