package com.fluxtream.connectors.freshbooks;
...
public class FreshbooksOAuthController {
	...
	public String getToken(HttpServletRequest request, HttpServletResponse response) throws IOException,
		...
		OAuthProvider provider = new DefaultOAuthProvider("https://" + userUrl
				+ ".freshbooks.com/oauth/oauth_request.php", "https://" + userUrl
				+ ".freshbooks.com/oauth/oauth_access.php", "https://" + userUrl
				+ ".freshbooks.com/oauth/oauth_authorize.php");
	...
	public String upgradeToken(HttpServletRequest request, HttpServletResponse response) throws OAuthException,
		...
		guestService.setApiKeyAttribute(guest.getId(), Connector.getConnector("FRESHBOOKS"), "accessToken",
				consumer.getToken());
		guestService.setApiKeyAttribute(guest.getId(), Connector.getConnector("FRESHBOOKS"), "tokenSecret",
				consumer.getTokenSecret());

		return "redirect:/analyze/from/" + Connector.getConnector("FRESHBOOKS").getName();
	}
