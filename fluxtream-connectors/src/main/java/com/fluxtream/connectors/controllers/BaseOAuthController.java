...
public abstract class BaseOAuthController {
	...
	protected String authorizeToken(HttpServletRequest request, String tokenSecretKey, Connector api,
			Map<String, String> extraParameters) throws OAuthException {
		GoogleOAuthParameters oauthParameters = new GoogleOAuthParameters();
		oauthParameters.setOAuthConsumerKey(getConsumerKey());
		oauthParameters.setOAuthConsumerSecret(getConsumerSecret());
		if (extraParameters != null) {
			for (Entry<String, String> extraParameter : extraParameters.entrySet())
				oauthParameters.addExtraParameter(extraParameter.getKey(), extraParameter.getValue());
		}
		String tokenSecret = (String) request.getSession().getAttribute(tokenSecretKey);
