...
public class LinkedInConnectorController {
	...
	public String getToken(HttpServletRequest request, HttpServletResponse response) throws IOException,
		...
		String oauthCallback = env.get("homeBaseUrl") + "linkedin/upgradeToken";
	...
	public String upgradeToken(HttpServletRequest request, HttpServletResponse response)
		...
		guestService.setApiKeyAttribute(guest.getId(), Connector.getConnector("linkedin"), "accessToken",
				consumer.getToken());
		guestService.setApiKeyAttribute(guest.getId(), Connector.getConnector("linkedin"), "tokenSecret",
				consumer.getTokenSecret());

		return "redirect:/analyze/from/" + Connector.getConnector("linkedin").getName();
	}
