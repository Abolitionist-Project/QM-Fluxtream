...
public class LastFmController {
	...
	public String getToken(HttpServletRequest request, HttpServletResponse response) throws IOException,
		...
		String oauthCallback = env.get("homeBaseUrl") + "lastfm/upgradeToken";
		if (request.getParameter("guestId") != null)
			oauthCallback += "?guestId=" + request.getParameter("guestId");

		String approvalPageUrl = "http://www.last.fm/api/auth/?api_key=" + env.get("lastfmConsumerKey") + "&cb="
				+ oauthCallback;
	...
	public String upgradeToken(HttpServletRequest request, HttpServletResponse response)
		...
		String api_sig = getApiSig(toMap("token", token, "api_key", api_key, "method", "auth.getsession"));

		Map<String, String> params = toMap("method", "auth.getsession", "format", "json", "token", token, "api_key",
				api_key, "api_sig", api_sig);
		String jsonResponse = HttpUtils.fetch("https://ws.audioscrobbler.com/2.0/", params);
		...
		return "redirect:/analyze/from/" + Connector.getConnector("LASTFM").getName();
	}
	...
	String getApiSig(Map<String, String> params) throws NoSuchAlgorithmException, UnsupportedEncodingException {
