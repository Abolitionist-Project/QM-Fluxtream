...
public class BodyMediaSleepFieldHandler implements FieldHandler {
	...
	public void handleField(final long guestId, AbstractFacet facet) {
		...
		for (int i = 0; i < sleepJson.size(); i++) {
			...
			if (!wasLying)
				addStandingRecord((facet.start / 1000) + lastMinuteIndex * 60, sleepingData, lyingData,
						(minuteIndex - lastMinuteIndex));
	...
	private void addSleepingRecord(long when, List<List<Object>> sleepingData, List<List<Object>> lyingData,
			int duration) {
	...
	private void addStandingRecord(long when, List<List<Object>> sleepingData, List<List<Object>> lyingData,
			int duration) {
		for (int i = 0; i < duration; i++) {
