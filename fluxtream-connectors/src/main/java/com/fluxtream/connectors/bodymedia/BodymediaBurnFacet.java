...
public class BodymediaBurnFacet extends BodymediaAbstractFacet {
	...
	public static AbstractFacet createOrUpdateDay(BodymediaAbstractFacet existing, final UpdateInfo updateInfo,
			JSONObject burnJson, DateTimeZone timeZone) {
		/*
		...
		if (burnJson.has("days") && burnJson.has("lastSync")) {
			...
			if (daysArray.size() != 1)
				throw new RuntimeException("days array is not the right length: expected 1, got "
						+ String.valueOf(daysArray.size()));
			...
			// Sets the start and end times for the facet so that it can be
			// uniquely defined
