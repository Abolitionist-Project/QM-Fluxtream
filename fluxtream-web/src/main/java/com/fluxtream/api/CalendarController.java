...
public class CalendarController {
	...
	public String getModel(@QueryParam("state") String state) throws IOException {
		...
		StringBuilder sb = new StringBuilder("module=API component=calendarController action=getModel").append(
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public String setToToday(@QueryParam("timeUnit") String timeUnit, @QueryParam("state") String state)
			throws IOException {
		...
		StringBuilder sb = new StringBuilder("module=API component=calendarController action=setToToday")
				.append(" guestId=").append(guestId).append(" state=").append(state);
		logger.info(sb.toString());
	...
	@Path(value = "/getWeek")
	@Produces({ MediaType.APPLICATION_JSON })
	public String setWeek(@QueryParam("state") String state, @QueryParam("year") int year, @QueryParam("week") int week)
			throws IOException {
		...
		StringBuilder sb = new StringBuilder("module=API component=calendarController action=getWeek")
				.append(" state=").append(state).append(" guestId=").append(guestId);
		logger.info(sb.toString());
		CalendarModel calendarModel = new CalendarModel(guestId, metadataService);
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public String getDateRangeForWeek(@QueryParam("week") int week, @QueryParam("year") int year) {
	...
	public String getDateWeek(@QueryParam("formattedDate") String formattedDate) {
		...
		// Need to adjust forward one week if the day of the week is a Sunday -
		// this is
		// the first day of the next week in CalendarModel, but is the last day
		// of the
		...
		return String.format("[%d, %d]", firstOfWeek.getWeekyear(), firstOfWeek.getWeekOfWeekyear());
	}

	@GET
	@Path(value = "/getMonth")
	@Produces({ MediaType.APPLICATION_JSON })
	public String setMonth(@QueryParam("state") String state, @QueryParam("year") int year,
			@QueryParam("month") int month) throws IOException {
	...
	@Path(value = "/getYear")
	@Produces({ MediaType.APPLICATION_JSON })
	public String setYear(@QueryParam("state") String state, @QueryParam("year") int year) throws IOException {
	...
	@Path(value = "/getDate")
	@Produces({ MediaType.APPLICATION_JSON })
	public String setDate(@QueryParam("date") String date) throws IOException {
		...
		StringBuilder sb = new StringBuilder("module=API component=calendarController action=setDate")
				.append(" guestId=").append(guestId).append(" date=").append(date);
		logger.info(sb.toString());
		CalendarModel calendarModel = new CalendarModel(guestId, metadataService);
