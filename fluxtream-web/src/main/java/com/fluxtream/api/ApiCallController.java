...
public class ApiCallController {
	...
	public String getFitbitBodyWeightHistory() {
		String formattedDate = dateFormat.withZone(DateTimeZone.forTimeZone(TimeZone.getDefault())).print(
		...
		try {
			String json = signpostHelper.makeRestCall(fitbitConnector, apiKey, 8,
					"http://api.fitbit.com/1/user/-/body/weight/date/" + formattedDate + "/max.json");
			return json;
	...
	public String getFitbitAriaWeightHistory() {
		...
		try {
			String json = signpostHelper.makeRestCall(fitbitConnector, apiKey, 8,
					"http://api.fitbit.com/1/user/-/body/log/weight/date/" + formattedDate + "/1m.json");
			return json;
		} catch (RateLimitReachedException e) {
			System.out.println(e.getMessage());
		}
	...
	// @Produces({ MediaType.APPLICATION_JSON })
	// public String handleBodytrack(@QueryParam("username") String username,
	// @QueryParam("connector") String conn) {
	// try{
	// Guest guest = guestService.getGuest(username);
	// AbstractFacet facet = jpaDaoService.findOne("bodymedia." + conn +
	// ".between", AbstractFacet.class, guest.getId(), 0L,
	// System.currentTimeMillis());
	...
	public String addBodymedia(@QueryParam("username") String username) {
		try {
			...
			try {
				approvalPageUrl = provider.retrieveRequestToken(consumer, oauthCallback);
