...
public class BodyTrackController {
	...
	public String loadHistory(@QueryParam("username") String username, @QueryParam("connectorName") String connectorName)
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public String uploadToBodytrack(@FormParam("dev_nickname") String deviceNickanme,
			@FormParam("channel_names") String channels, @FormParam("data") String data) {
		setTransactionName(null, "POST /bodytrack/upload");
		StatusModel status;
		try {
			long uid = AuthHelper.getGuestId();
			Type channelsType = new TypeToken<Collection<String>>() {
			}.getType();
			Type dataType = new TypeToken<List<List<Long>>>() {
			}.getType();

			final BodyTrackHelper.BodyTrackUploadResult uploadResult = bodyTrackHelper.uploadToBodyTrack(uid,
					deviceNickanme, (Collection<String>) gson.fromJson(channels, channelsType),
					(List<List<Object>>) gson.fromJson(data, dataType));
	...
	public String uploadJsonToBodytrack(@QueryParam("dev_nickname") String deviceNickname, String body) {
		...
		try {
			long uid = AuthHelper.getGuestId();
			status = createStatusModelFromBodyTrackUploadResult(bodyTrackHelper.uploadJsonToBodyTrack(uid,
					deviceNickname, body));
		} catch (Exception e) {
			status = new StatusModel(false, "Upload failed!");
		}
	...
	private StatusModel createStatusModelFromBodyTrackUploadResult(
			final BodyTrackHelper.BodyTrackUploadResult uploadResult) {

		// check the uploadResult for success, and create a new StatusModel
		// accordingly
		...
		// Now try to parse the response in the uploadResult as JSON, inflating
		// it into a BodyTrackUploadResponse
		...
		try {
			...
			LOG.error(
					"JsonSyntaxException while trying to convert the BodyTrackUploadResult response into a BodyTrackUploadResponse.  Response was ["
							+ uploadResult.getResponse() + "]", e);
		}
	...
	// and http://stackoverflow.com/a/4687942
	@POST
	@Path("/photoUpload")
	@Consumes({ MediaType.MULTIPART_FORM_DATA })
	@Produces({ MediaType.APPLICATION_JSON })
	public Response handlePhotoUpload(@QueryParam("connector_name") final String connectorName,
		...
		if (connector != null) {
			// We currently only support photo uploads for the Fluxtream Capture
			// connector
			if (FluxtreamCaptureUpdater.CONNECTOR_NAME.equals(connector.getName())) {
				try {
					if (multiPart != null) {
						...
						// iterate over the body parts and pick out the "photo"
						// and "metadata" parts
						...
						if (photoBytes != null && jsonMetadata != null) {
							...
							// Record the upload request time. In reality we
							// don't really care about the upload
							// time, but rather we want to ensure that the
							// Fluxtream Capture connector is
							// auto-added to the user's set of connectors, and
							// this is the way to do it. Note that the
							// the field recorded here is merely the time of the
							// last upload request *request* and says
							// nothing about whether that request was actually
							// successful
							guestService.setApiKeyAttribute(guestId, connector, "last_upload_request_time",
									String.valueOf(System.currentTimeMillis()));

							// We have a photo and the metadata, so pass control
							// to the FluxtreamCapturePhotoStore to save the
							// photo
							LOG_DEBUG.debug("BodyTrackController.savePhoto(" + guestId + ", " + photoBytes.length
									+ ", " + jsonMetadata + ")");
							try {
								final FluxtreamCapturePhotoStore.OperationResult<FluxtreamCapturePhoto> result = fluxtreamCapturePhotoStore
										.saveOrUpdatePhoto(guestId, photoBytes, jsonMetadata);
								final String photoStoreKey = result.getData().getPhotoStoreKey();
								LOG.info("BodyTrackController.handlePhotoUpload(): photo [" + photoStoreKey + "] "
										+ result.getOperation() + " sucessfully!");
								response = jsonResponseHelper.ok("photo " + result.getOperation() + " sucessfully!",
										new PhotoUploadPayload(result.getOperation(), photoStoreKey));
	...
	public Response getFluxtreamCapturePhoto(@PathParam("UID") final Long uid,
			@PathParam("PhotoStoreKeySuffix") final String photoStoreKeySuffix, @Context final Request request) {
	...
	public Response getFluxtreamCapturePhotoThumbnail(@PathParam("UID") final long uid,
			@PathParam("PhotoId") final long photoId, @PathParam("ThumbnailIndex") final int thumbnailIndex,
	...
	private Response getFluxtreamCapturePhoto(final long uid, final Request request,
			@NotNull final FluxtreamCapturePhotoFetchStrategy photoFetchStrategy) {

		// Check authorization: is the logged-in user the same as the UID in the
		// key? If not, does the logged-in user
		...
		if (accessAllowed) {
			...
			try {
				...
				final String message = "Exception while trying to get photo ["
						+ photoFetchStrategy.getPhotoIdentifier() + "]";
				LOG.error("BodyTrackController.getFluxtreamCapturePhoto(): " + message, e);
				return jsonResponseHelper.internalServerError(message);
			}

			if (photo == null) {
				final String message = "Photo [" + photoFetchStrategy.getPhotoIdentifier() + "] requested by user ["
						+ loggedInUserId + "] not found";
			...
			try {
				...
				if (responseBuilder != null) {
					// Preconditions are not met, returning HTTP 304
					// 'not-modified'
					return responseBuilder.cacheControl(cc).build();
				}
			} catch (NoSuchAlgorithmException e) {
				LOG.warn("NoSuchAlgorithmException caught while trying to create an MD5 hash for photo ["
						+ photo.getIdentifier() + "].  No Etag will be specified in the response.");
				etag = null;
			}
			...
			return responseBuilder.type(photo.getImageType().getMediaType())
					.expires(new DateTime().plusMonths(1).toDate()).entity(photo.getPhotoBytes()).build();
		}

		return jsonResponseHelper.forbidden("User [" + loggedInUserId + "] is not authorized to view photo ["
				+ photoFetchStrategy.getPhotoIdentifier() + "]");
	}
	...
	public String fetchTile(@PathParam("UID") Long uid, @PathParam("DeviceNickname") String deviceNickname,
			@PathParam("ChannelName") String channelName, @PathParam("Level") int level,
			@PathParam("Offset") long offset) {
		setTransactionName(null, "GET /bodytrack/tiles/{UID}/" + deviceNickname + "." + channelName
				+ "/{Level}.{Offset}.json");
	...
	public String fetchPhotoTile(@PathParam("UID") Long uid,
			...
			@PathParam("ObjectTypeName") String objectTypeName, @PathParam("Level") int level,
			@PathParam("Offset") long offset) {
		setTransactionName(null, "GET /bodytrack/photos/{UID}/" + connectorPrettyName + "." + objectTypeName
				+ "/{Level}.{Offset}.json");
		long loggedInUserId = AuthHelper.getGuestId();
		boolean accessAllowed = checkForPermissionAccess(uid);
		CoachingBuddy coachee = coachingService.getCoachee(loggedInUserId, uid);
		...
		try {
			...
			// first use Level and Offset to calculate the desired start and end
			// times
			...
			final TimeInterval timeInterval = new TimeInterval(startTimeMillis, endTimeMillis, TimeUnit.DAY,
					TimeZone.getTimeZone("UTC"));

			// fetch the photos for this time interval, and for the desired
			// device/channel
			final SortedSet<PhotoService.Photo> photos = photoService.getPhotos(uid, timeInterval, connectorPrettyName,
					objectTypeName);

			// Define the min interval to be 1/20th of the span of the tile.
			// Value is in seconds
			...
			// * min_interval specifies the minimum number of seconds between
			// images.
			// * Always include the first photo, set count to 1
			// * When processing a given photo B, compare the time of this photo
			// with the previous included
			// photo A. If image B is < min_interval seconds after image A, then
			// increase count field in image
			// A and ignore image B. If image B is >= min_interval seconds after
			// image A, then include image
			// B with count=1
			...
			for (final PhotoService.Photo photoB : photos) {
				if (photoA == null) {
					...
					// Already have a photoA, compare times to see if we should
					// keep this one
					...
					if (photoBStartTimeSecs > (photoA.begin_d + minInterval)) {
						// Enough of a gap between A and B, so keep this one and
						// set to be new A
						photoA = new PhotoItem(photoB);
						filteredPhotos.add(photoA);
					} else {
			...
			if (LOG_DEBUG.isDebugEnabled()) {
				LOG_DEBUG.debug("BodyTrackController.fetchPhotoTile(): num photos filtered from " + photos.size()
						+ " to " + filteredPhotos.size());
			}
	...
	public String getPhotosBeforeOrAfterTime(@PathParam("UID") long uid,
			@PathParam("ConnectorPrettyName") String connectorPrettyName,
			@PathParam("ObjectTypeName") String objectTypeName, @PathParam("unixTime") double unixTimeInSecs,
			@PathParam("count") int desiredCount, @QueryParam("isBefore") boolean isGetPhotosBeforeTime,
			@QueryParam("tags") List<String> tags, @QueryParam("isMatchAllTags") boolean isMatchAllTags) {
		setTransactionName(null, "GET /bodytrack/photos/{UID}/" + connectorPrettyName + "." + objectTypeName
				+ "/{unixTime}/{count}");
		long loggedInUserId = AuthHelper.getGuestId();
		boolean accessAllowed = checkForPermissionAccess(uid);
		...
		try {
			...
			final SortedSet<PhotoService.Photo> photos = photoService.getPhotos(uid, (long) (unixTimeInSecs * 1000),
					connectorPrettyName, objectTypeName, desiredCount, isGetPhotosBeforeTime);
			...
			LOG.error("BodyTrackController.getPhotosBeforeOrAfterTime(): Exception while trying to fetch log items: ",
					e);
			return gson.toJson(new StatusModel(false, "Access Denied"));
		}
	}
	...
	private static class PhotoItem {
		...
		public PhotoItem(final PhotoService.Photo photo) {
			...
			this.begin_d = photoFacetVO.start / 1000.0; // convert millis to
														// seconds
			...
			this.channel_name = PhotoService.DEFAULT_PHOTOS_CHANNEL_NAME; // photo
																			// channels
																			// are
																			// always
																			// named
	...
	private static final class LevelOffsetHelper {
		...
		/**
		 * Returns the time in seconds of a bin in a tile at a given level. This
		 * is 2^level seconds, so level 0 bins are 1 second, level 4 are 16
		 * secs, etc.
		 */
		...
		/**
		 * Returns the duration in seconds of a tile at a given level. For level
		 * 0 this is TILE_BIN_NUM seconds.
		 */
		 ...
		 * level. This is the duration at that level times the offset.
		 */
	...
	private static final class BodyTrackUploadResponse {
		// We only store the bare minimum here because it might be a
		// security/privacy issue to include everything (Randy
		// explained to Chris on 2012.10.31 that we probably don't want to make
		// channel ranges and such visible by
		// default. Plus, if debugging is on in the datastore, file paths might
		// also be included in the response JSON).
	...
	private class PhotoUploadPayload {
		...
		public PhotoUploadPayload(@NotNull final FluxtreamCapturePhotoStore.Operation operation,
				@NotNull final String photoStoreKey) {
