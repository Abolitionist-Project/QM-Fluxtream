...
public class PhotoStore {
	...
	@Path("/week/{year}/{week}")
	@Produces({ MediaType.APPLICATION_JSON })
	public String getPhotosForWeek(@PathParam("username") String username, @PathParam("year") int year,
		...
		try {
			...
			DayMetadataFacet dayMetaEnd = metadataService.getDayMetadata(
					...
					year + "-" + datePartFormat.format(c.get(Calendar.MONTH) + 1) + "-"
							+ datePartFormat.format(c.get(Calendar.DAY_OF_MONTH)), true);
			return gson.toJson(getPhotos(guest, new TimeInterval(dayMetaStart.start, dayMetaEnd.end, TimeUnit.WEEK,
					TimeZone.getTimeZone(dayMetaStart.timeZone))));
		} catch (Exception e) {
	...
	private List<PhotoModel> getPhotos(Guest guest, TimeInterval timeInterval) throws ClassNotFoundException,
