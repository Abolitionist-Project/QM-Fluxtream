...
public class GuestController {
	...
	public String getCurrentGuest() throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		try {
	...
	public String setOAuthTokens(@PathParam("connector") String connectorName,
			@FormParam("accessToken") String accessToken, @FormParam("tokenSecret") String tokenSecret)
		...
		try {
			...
			if (apiKey != null) {
				...
				StatusModel result = new StatusModel(true, "Successfully updated oauth tokens: " + connectorName);
				return gson.toJson(result);
			} else {
				StatusModel result = new StatusModel(false, "Guest does not have that connector: " + connectorName);
				return gson.toJson(result);
	...
	public String setOAuth2Tokens(@PathParam("connector") String connectorName,
		...
		try {
			...
			if (apiKey != null) {
				...
				guestService.setApiKeyAttribute(guestId, apiKey.getConnector(), "refreshTokenRemoveURL",
						refreshTokenRemoveURL);
				guestService.setApiKeyAttribute(guestId, apiKey.getConnector(), "refreshToken", refreshToken);

				StatusModel result = new StatusModel(true, "Successfully updated oauth2 tokens: " + connectorName);
				return gson.toJson(result);
			} else {
				StatusModel result = new StatusModel(false, "Guest does not have that connector: " + connectorName);
				return gson.toJson(result);
	...
	@Path("/create")
	@Produces({ MediaType.APPLICATION_JSON })
	public String createGuest(@FormParam("username") String username, @FormParam("firstname") String firstname,
	...
	@Path("/{username}")
	@Produces({ MediaType.APPLICATION_JSON })
	public String deleteGuest(@PathParam("username") String username) throws InstantiationException,
			IllegalAccessException, ClassNotFoundException {
		try {
			guestService.eraseGuestInfo(username);
			StatusModel result = new StatusModel(true, "User " + username + " was successfully deleted");
			return gson.toJson(result);
		} catch (Exception e) {
			StatusModel result = new StatusModel(false, "Could not delete guest: " + e.getMessage());
			return gson.toJson(result);
		}
	...
	@Path("/list")
	@Produces({ MediaType.APPLICATION_JSON })
	public String list() throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		try {
			...
			for (Guest guest : list) {
				...
				guestJson.accumulate("id", guest.getId()).accumulate("username", guest.username)
						.accumulate("firstname", guest.firstname).accumulate("lastname", guest.lastname)
						.accumulate("email", guest.email).accumulate("roles", guest.getUserRoles());
				array.add(guestJson);
			}
	...
	@Path("/{username}/roles")
	@Produces({ MediaType.APPLICATION_JSON })
	public String getRoles(@PathParam("username") String username) throws InstantiationException,
			IllegalAccessException, ClassNotFoundException {
		try {
			...
			StatusModel result = new StatusModel(false, "Could not get roles: " + e.getMessage());
			return gson.toJson(result);
		}
	...
	@Path("/{username}/roles")
	@Produces({ MediaType.APPLICATION_JSON })
	public String setRoles(@PathParam("username") String username, @FormParam("roles") String roles)
			throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		try {
			Guest guest = guestService.getGuest(username);
			...
			while (st.hasMoreTokens()) {
				...
				for (String existingRole : userRoles) {
					if (existingRole.toLowerCase().equals(newRole.toLowerCase()))
						continue;
				}
			...
			result.accumulate("result", "OK")
					.accumulate("message", "successfully added role " + StringUtils.join(addedRoles, ", "))
					.accumulate("user_roles:", array);
			return result.toString();
		} catch (Exception e) {
			StatusModel result = new StatusModel(false, "Could not grant role: " + e.getMessage());
			return gson.toJson(result);
		}
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public String revokeRole(@PathParam("username") String username, @PathParam("role") String role)
			throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		try {
			Guest guest = guestService.getGuest(username);
			...
			JSONArray array = getGuestRolesJsonArray(guest);
			JSONObject result = new JSONObject();
			result.accumulate("result", "OK").accumulate("message", "successfully removed role " + role)
					.accumulate("user_roles:", array);
			return result.toString();
		} catch (Exception e) {
			StatusModel result = new StatusModel(false, "Could not revoke role: " + e.getMessage());
			return gson.toJson(result);
		}
