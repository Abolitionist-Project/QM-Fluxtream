...
public class CalendarDataHelper {
	...
	public List<AbstractFacet> getFacets(Connector connector, ObjectType objectType, List<String> dates) {
		List<AbstractFacet> facets = new ArrayList<AbstractFacet>();
		try {
			if (AuthHelper.isViewingGranted(connector.getName()))
				facets = apiDataService.getApiDataFacets(AuthHelper.getVieweeId(), connector, objectType, dates);
		} catch (Throwable t) {
			t.printStackTrace();
		}
		return facets;
	...
	public List<AbstractFacet> getFacets(Connector connector, ObjectType objectType, DayMetadataFacet dayMetadata,
			int lookbackDays) {
		List<AbstractFacet> facets = new ArrayList<AbstractFacet>();
		try {
			if (AuthHelper.isViewingGranted(connector.getName()))
				facets = apiDataService.getApiDataFacets(AuthHelper.getVieweeId(), connector, objectType,
	...
	void refreshApiData(DayMetadataFacet dayMetadata, ApiKey apiKey, ObjectType objectType, ConnectorResponseModel crm) {
		...
		if (objectType == null && objectTypeValues != null && objectTypeValues.length > 0) {
	...
	public void updateObjectTypeData(ApiKey apiKey, int objectTypes, ConnectorResponseModel crm, TimeInterval interval) {
		UpdateStrategy updateStrategy = updateStrategyFactory.getUpdateStrategy(apiKey.getConnector());
		UpdateInfo updateInfo = updateStrategy.getUpdateInfo(apiKey, objectTypes, interval);
		// either we need to initiate a history update
		if (updateInfo.getUpdateType() == UpdateInfo.UpdateType.INITIAL_HISTORY_UPDATE) {
			ScheduleResult scheduleResult = connectorUpdateService.scheduleUpdate(apiKey.getGuestId(), apiKey
					.getConnector().getName(), objectTypes, UpdateInfo.UpdateType.INITIAL_HISTORY_UPDATE, System
					.currentTimeMillis());
		...
		// or, unless we were told not to (NOOP), we update the api data
			...
			UpdateResult updateResult = doUpdateApiData(apiKey, objectTypes, updateInfo);
			crm.addUpdateResult(updateResult);
		}
	}

	UpdateResult doUpdateApiData(ApiKey apiKey, int objectTypes, UpdateInfo updateInfo) {
