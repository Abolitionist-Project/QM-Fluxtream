...
public class SyncController {
	...
	private String sync(final String connectorName, final boolean force) {
		try {
			final List<ScheduleResult> scheduleResults = connectorUpdateService.updateConnector(
					AuthHelper.getGuestId(), Connector.getConnector(connectorName), force);
			StatusModel statusModel = new StatusModel(true,
					"successfully added update worker tasks to the queue (see details)");
	...
	private String syncConnectorObjectType(final String connectorName, final int objectTypes, final boolean force) {
		try {
			final List<ScheduleResult> scheduleResults = connectorUpdateService.updateConnectorObjectType(
					AuthHelper.getGuestId(), Connector.getConnector(connectorName), objectTypes, force);
			StatusModel statusModel = new StatusModel(true,
					"successfully added update worker tasks to the queue (see details)");
			statusModel.payload = scheduleResults;
			return gson.toJson(scheduleResults);
	...
	public String updateAllConnectors() {
		...
		try {
			final List<ScheduleResult> scheduleResults = connectorUpdateService.updateAllConnectors(
					AuthHelper.getGuestId(), false);
			StatusModel statusModel = new StatusModel(true,
					"successfully added update worker tasks to the queue (see details)");
			statusModel.payload = scheduleResults;
			return gson.toJson(scheduleResults);
	...
	public String isHistoryComplete(@PathParam("connector") String connectorName,
		...
		final boolean historyUpdateCompleted = connectorUpdateService.isHistoryUpdateCompleted(user.getId(),
	...
	public String isSynching(@PathParam("connector") String connectorName) {
		...
		final Collection<UpdateWorkerTask> scheduledUpdates = connectorUpdateService.getUpdatingUpdateTasks(
				guest.getId(), connector);
		JSONObject response = new JSONObject();
	...
	public String lastSuccessfulUpdate(@PathParam("connector") String connectorName) {
		...
		JSONObject response = new JSONObject();
		response.accumulate("lastSuccessfulUpdate", lastSuccessfulUpdate != null ? fmt.print(lastSuccessfulUpdate.ts)
				: "never");
		return response.toString();
	}
	...
	public String lastFailedUpdate(@PathParam("connector") String connectorName) {
		...
		JSONObject response = new JSONObject();
		response.accumulate("lastUpdate", lastUpdate != null ? fmt.print(lastUpdate.ts) : "never");
		return response.toString();
	}
