...
public class CalendarDataStore {
	...
	public String getAllConnectorsContinuous() throws InstantiationException, IllegalAccessException,
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public String getAllConnectorsWeekData(@PathParam("year") int year, @PathParam("week") int week,
			@QueryParam("filter") String filter) throws InstantiationException, IllegalAccessException,
			ClassNotFoundException {
		...
		try {
			...
			// this implementation is just a dirt hacky way to make it work and
			// some aspects (weather info) don't work
			...
			DayMetadataFacet dayMetaStart = metadataService.getDayMetadata(
					guest.getId(),
					year + "-" + datePartFormat.format(c.get(Calendar.MONTH) + 1) + "-"
			...
			DayMetadataFacet dayMetaEnd = metadataService.getDayMetadata(
					guest.getId(),
					year + "-" + datePartFormat.format(c.get(Calendar.MONTH) + 1) + "-"
							+ datePartFormat.format(c.get(Calendar.DAY_OF_MONTH)), true);
			...
			digest.timeZoneOffset = TimeZone.getTimeZone(dayMetadata.timeZone).getOffset(
					(digest.tbounds.start + digest.tbounds.end) / 2);

			City city = metadataService.getMainCity(guestId, dayMetadata);

			/*
			 * if (city != null){ well digest.hourlyWeatherData =
			 * metadataService
			 * .getWeatherInfo(city.geo_latitude,city.geo_longitude, date, 0, 24
			 * * 60); Collections.sort(digest.hourlyWeatherData); }
			 */
			...
			setCachedData(digest, allApiKeys, settings, apiKeySelection, dayMetadata);
			...
			StringBuilder sb = new StringBuilder(
					"module=API component=calendarDataStore action=getAllConnectorsWeekData").append(" year=")
					.append(year).append(" week=").append(week).append(" timeTaken=")
					.append(System.currentTimeMillis() - then).append(" guestId=").append(guestId);
			...
			StringBuilder sb = new StringBuilder(
					"module=API component=calendarDataStore action=getAllConnectorsWeekData").append(" year=")
					.append(year).append(" week=").append(week).append(" guestId=").append(guestId)
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public String getAllConnectorsMonthData(@PathParam("year") int year, @PathParam("month") int month,
			@QueryParam("filter") String filter) throws InstantiationException, IllegalAccessException,
			ClassNotFoundException {
		...
		try {
			...
			DayMetadataFacet dayMetaStart = metadataService.getDayMetadata(guest.getId(), year + "-" + month + "-01",
					true);

			DayMetadataFacet dayMetaEnd = metadataService.getDayMetadata(guest.getId(), year + "-" + month + "-"
					+ endDayNum, true);
			...
			/*
			 * if (city != null){ digest.hourlyWeatherData =
			 * metadataService.getWeatherInfo
			 * (city.geo_latitude,city.geo_longitude, date, 0, 24 * 60);
			...
			StringBuilder sb = new StringBuilder(
					"module=API component=calendarDataStore action=getAllConnectorsMonthData").append(" year=")
					.append(year).append(" month=").append(month).append(" timeTaken=")
					.append(System.currentTimeMillis() - then).append(" guestId=").append(guestId);
			logger.info(sb.toString());
			...
			StringBuilder sb = new StringBuilder(
					"module=API component=calendarDataStore action=getAllConnectorsMonthData").append(" year=")
					.append(year).append(" month=").append(month).append(" guestId=").append(guestId)
					.append(" stackTrace=<![CDATA[").append(Utils.stackTrace(e)).append("]]>");
			logger.warn(sb.toString());
			return gson.toJson(new StatusModel(false, "Failed to get digest: " + e.getMessage()));
		}
	...
	@Path("/all/year/{year}")
	@Produces({ MediaType.APPLICATION_JSON })
	public String getAllConnectorsYearData(@PathParam("year") int year, @QueryParam("filter") String filter)
		...
		try {
			...
			setCachedData(digest, userKeys, settings, apiKeySelection, dayMetadata);
			...
			StringBuilder sb = new StringBuilder(
					"module=API component=calendarDataStore action=getAllConnectorsYearData").append(" year=")
					.append(year).append(" timeTaken=").append(System.currentTimeMillis() - then).append(" guestId=")
					.append(guestId);
			logger.info(sb.toString());
			return gson.toJson(digest);
		} catch (Exception e) {
			StringBuilder sb = new StringBuilder(
					"module=API component=calendarDataStore action=getAllConnectorsYearData").append(" year=")
					.append(year).append(" guestId=").append(guestId);
			logger.warn(sb.toString());
			return gson.toJson(new StatusModel(false, "Failed to get digest: " + e.getMessage()));
		}
	}
	...
	@Path("/all/date/{date}")
	@Produces({ MediaType.APPLICATION_JSON })
	public String getAllConnectorsDayData(@PathParam("date") String date, @QueryParam("filter") String filter)
			throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		...
		try {
			...
			return gson.toJson(new StatusModel(false,
		...
		try {
			...
			if (city != null) {
				digest.hourlyWeatherData = metadataService.getWeatherInfo(city.geo_latitude, city.geo_longitude, date,
						0, 24 * 60);
				Collections.sort(digest.hourlyWeatherData);
			}
			...
			StringBuilder sb = new StringBuilder(
					"module=API component=calendarDataStore action=getAllConnectorsDayData").append(" date=")
					.append(date).append(" timeTaken=").append(System.currentTimeMillis() - then).append(" guestId=")
					.append(guestId);
			logger.info(sb.toString());
			return gson.toJson(digest);
		} catch (Exception e) {
			StringBuilder sb = new StringBuilder(
					"module=API component=calendarDataStore action=getAllConnectorsYearData").append(" date=")
					.append(date).append(" guestId=").append(guestId);
			logger.warn(sb.toString());
			return gson.toJson(new StatusModel(false, "Failed to get digest: " + e.getMessage()));
		}
	}
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public String getConnectorData(@PathParam("date") String date, @PathParam("connectorName") String connectorName)
			throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		...
		try {
			...
			if (objectTypes != null) {
				for (ObjectType objectType : objectTypes) {
					...
					if (objectType.isDateBased())
						facetCollection = getFacetVos(Arrays.asList(date), settings, connector, objectType,
			...
			StringBuilder sb = new StringBuilder("module=API component=calendarDataStore action=getConnectorData")
					.append(" date=").append(date).append(" connector=").append(connectorName).append(" timeTaken=")
					.append(System.currentTimeMillis() - then).append(" guestId=").append(guestId);
			logger.info(sb.toString());
	...
	private ConnectorResponseModel prepareConnectorResponseModel(DayMetadataFacet dayMetadata) {
		TimeBoundariesModel tb = calendarDataHelper.getStartEndResponseBoundaries(dayMetadata);
	...
	@SuppressWarnings("rawtypes")
	private void setCachedData(DigestModel digest, List<ApiKey> userKeys, GuestSettings settings,
			List<ApiKey> apiKeySelection, DayMetadataFacet dayMetadata) throws InstantiationException,
		...
		for (ApiKey apiKey : userKeys) {
			...
			if (objectTypes != null) {
				for (ObjectType objectType : objectTypes) {
					Collection<AbstractFacetVO<AbstractFacet>> facetCollection = null;
					if (objectType.isDateBased())
						facetCollection = getFacetVos(
								toDates(dayMetadata.start, dayMetadata.end, TimeZone.getTimeZone(dayMetadata.timeZone)),
								settings, connector, objectType, dayMetadata.getTimeInterval());
					else
						facetCollection = getFacetVos(dayMetadata, settings, connector, objectType);

					setFilterInfo(dayMetadata, digest, apiKeySelection, apiKey, connector, objectType, facetCollection);
				}
			} else {
				Collection<AbstractFacetVO<AbstractFacet>> facetCollection = getFacetVos(dayMetadata, settings,
						connector, null);
				setFilterInfo(dayMetadata, digest, apiKeySelection, apiKey, connector, null, facetCollection);
			}
		}
	}
	...
	@SuppressWarnings("rawtypes")
	private void setFilterInfo(DayMetadataFacet dayMetadata, DigestModel digest, List<ApiKey> apiKeySelection,
			ApiKey apiKey, Connector connector, ObjectType objectType,
	...
	private Collection<AbstractFacetVO<AbstractFacet>> getFacetVos(List<String> dates, GuestSettings settings,
			Connector connector, ObjectType objectType, TimeInterval timeInterval) throws InstantiationException,
			IllegalAccessException, ClassNotFoundException {
		List<AbstractFacet> objectTypeFacets = calendarDataHelper.getFacets(connector, objectType, dates);
	...
	private Collection<AbstractFacetVO<AbstractFacet>> getAbstractFacetVOs(final GuestSettings settings,
			final List<AbstractFacet> objectTypeFacets, final TimeInterval timeInterval) throws InstantiationException,
			IllegalAccessException, ClassNotFoundException {
		...
		if (objectTypeFacets != null) {
			for (AbstractFacet abstractFacet : objectTypeFacets) {
				AbstractFacetVO<AbstractFacet> facetVO = AbstractFacetVO.getFacetVOClass(abstractFacet).newInstance();
	...
	private Collection<AbstractFacetVO<AbstractFacet>> getFacetVos(DayMetadataFacet dayMetadata,
			GuestSettings settings, Connector connector, ObjectType objectType) throws InstantiationException,
			IllegalAccessException, ClassNotFoundException {
		List<AbstractFacet> objectTypeFacets = calendarDataHelper.getFacets(connector, objectType, dayMetadata,
				getLookbackDays(connector.getName(), objectType == null ? null : objectType.getName()));
	...
	private boolean needsUpdate(ApiKey apiKey, DayMetadataFacet dayMetadata) {
		...
		UpdateStrategy updateStrategy = updateStrategyFactory.getUpdateStrategy(apiKey.getConnector());
		if (updateStrategy == null) {
			return false;
		}
		List<UpdateInfo> updateInfos = getUpdateInfos(updateStrategy, apiKey, interval);
	...
	private List<UpdateInfo> getUpdateInfos(UpdateStrategy updateStrategy, ApiKey apiKey, TimeInterval interval) {
		...
		if (objectTypeValues != null && objectTypeValues.length > 0) {
			for (int i = 0; i < objectTypeValues.length; i++) {
				UpdateInfo updateInfo = updateStrategy.getUpdateInfo(apiKey, objectTypeValues[i], interval);
				updateInfos.add(updateInfo);
			}
		} else {
			UpdateInfo updateInfo = updateStrategy.getUpdateInfo(apiKey, -1, interval);
			updateInfos.add(updateInfo);
		}
		return updateInfos;
	}
	...
	private void setVisitedCities(DigestModel digest, long guestId, DayMetadataFacet md) {
		...
		if (orderedCities != null) {
			NavigableSet<VisitedCity> descendingSet = orderedCities.descendingSet();
	...
	private List<ApiKey> getApiKeySelection(long guestId, String filter, CoachingBuddy coachee) {
		...
		if (uncheckedConnectors != null && uncheckedConnectors.length > 0) {
			filteredOutConnectors = new ArrayList<String>(Arrays.asList(uncheckedConnectors));
		}
		List<ApiKey> apiKeySelection = getCheckedApiKeys(userKeys, filteredOutConnectors, coachee);
		return apiKeySelection;
	}

	private List<ApiKey> getCheckedApiKeys(List<ApiKey> apiKeys, List<String> uncheckedConnectors, CoachingBuddy coachee) {
		...
		there: for (ApiKey apiKey : apiKeys) {
			...
			// Check if apiKey should be skipped due to being in
			// uncheckedConnectors list.
