...
public class AddressStore {
	...
	public String getAddressesOfTypeAtDate(@PathParam("type") String type, @PathParam("date") String date) {
		...
		try {
			Guest guest = AuthHelper.getGuest();
			DayMetadataFacet dayMeta = metadataService.getDayMetadata(guest.getId(), date, true);
			List<GuestAddress> addresses = settingsService.getAllAddressesOfTypeForDate(guest.getId(), type,
					(dayMeta.start + dayMeta.end) / 2);
			return gson.toJson(addresses);
		} catch (Exception e) {
			StatusModel result = new StatusModel(false, "Could not get guest addresses: " + e.getMessage());
			return gson.toJson(result);
	...
	public String addAddress(String type, String address, double latitude, double longitude, double radius,
			String since, String until, String username) {
		try {
			...
			String jsonString = HttpUtils
					.fetch("https://maps.googleapis.com/maps/api/geocode/json?sensor=false&address=" + addressEncoded);

			GuestAddress newAddress;

			if (until != null)
				newAddress = settingsService.addAddress(guest.getId(), type, address, latitude, longitude, startTime,
						endTime, radius, jsonString);
			else
				newAddress = settingsService.addAddress(guest.getId(), type, address, latitude, longitude, startTime,
						radius, jsonString);
	...
	@Path("/{input}")
	@Produces({ MediaType.APPLICATION_JSON })
	public String addUpdateAddress(@PathParam("input") String input, @FormParam("address") String address,
			@FormParam("latitude") @DefaultValue("91") double latitude,
			@FormParam("longitude") @DefaultValue("181") double longitude, @FormParam("since") String since,
			@FormParam("until") String until, @FormParam("type") String newType,
		...
		try {
			...
			try {
				return addAddress(input, address, latitude, longitude, radius, since, until,
						AuthHelper.getGuest().username);
			} catch (Exception e1) {
	...
	public String deleteAddress(@PathParam("index") int index) {
		...
		try {
			Guest guest = AuthHelper.getGuest();
			settingsService.deleteAddressById(guest.getId(),
					settingsService.getAllAddresses(guest.getId()).get(index).id);
	...
	@Path("/{type}/{date}/{index}")
	@Produces({ MediaType.APPLICATION_JSON })
	public String deleteAddressOfTypeAtDate(@PathParam("type") String type, @PathParam("date") String date,
			@PathParam("index") int index) {
	...
	public String updateAddress(String username, int index, String address, double latitude, double longitude,
			double radius, String since, String until, String newType) {
		StatusModel result;
		try {
			Guest guest = guestService.getGuest(username);
			...
			if (address != null) {
				String addressEncoded = URLEncoder.encode(address, "UTF-8");
				jsonString = HttpUtils.fetch("https://maps.googleapis.com/maps/api/geocode/json?sensor=false&address="
						+ addressEncoded);
			}
			...
			GuestAddress updatedAddress = settingsService.updateAddress(guest.getId(), add.id, newType, address,
					latitude > 90 ? null : latitude, longitude > 180 ? null : longitude, startTime, endTime,
					radius < 0 ? null : radius, jsonString);
	...
	@Path("/{type}/{date}/{index}")
	@Produces({ MediaType.APPLICATION_JSON })
	public String updateAddressOfTypeAtDate(@PathParam("type") String type, @PathParam("date") String date,
			@PathParam("index") int index) {
