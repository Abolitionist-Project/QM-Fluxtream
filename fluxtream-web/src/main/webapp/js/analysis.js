...
var fullInputVariableInfo, fullOutputVariableInfo;
...
function updateValuesAndDrawGraphs() {
	...
	var inputVarId = $('#input_vars_select option:selected').val();
	var outputVarId = $('#output_vars_select option:selected').val();
...
function retrieveValuesAndDrawGraphs(inputVarType, inputVarId, outputVarType, outputVarId, fromDate, toDate, grouping){
	$.when(
			$.getJSON('/variables/values?var_type=' + inputVarType + '&var_id=' + inputVarId + "&from=" + fromDate
					+ "&to=" + toDate + "&grouping=" + grouping, function(json) {
				fullInputVariableInfo = json;
			}),
			$.getJSON('/variables/values?var_type=' + outputVarType + '&var_id=' + outputVarId + "&from=" + fromDate
					+ "&to=" + toDate + "&grouping=" + grouping, function(json) {
				fullOutputVariableInfo = json;
			})).done(function() {
		// $('#show_original').click();
		showOriginal();
...
function drawPearsonCoeff(pearsonCoeff) {
	...
	var customizedSigMessage = significanceMessage.replace("{1}", pearsonCoeffSignificance).replace("{2}",
			fullInputVariableInfo.name).replace("{3}", fullOutputVariableInfo.name);
...
function drawTimeLineChart(xValues, yValues) {
	if (timeShiftedValue == null) {
		timeLineOptions.series[0].name = fullInputVariableInfo.name + ", " + fullInputVariableInfo.unitDto.name;
		...
		timeLineOptions.yAxis[0].title.text = fullInputVariableInfo.name;
		timeLineOptions.yAxis[1].title.text = fullOutputVariableInfo.name;
...
function drawScatterplot() {
	...
	scatterplotOptions.xAxis.title.text = fullInputVariableInfo.name;
	scatterplotOptions.yAxis.title.text = fullOutputVariableInfo.name;
	scatterplotOptions.tooltip.formatter = function() {
		return '' + this.x + ' ' + fullInputVariableInfo.unitDto.name + ', ' + this.y + ' ' + fullOutputVariableInfo.unitDto.name;
	};
