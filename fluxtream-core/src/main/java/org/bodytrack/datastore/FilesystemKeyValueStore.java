...
/**
 * <p>
 * <code>FilesystemKeyValueStore</code> is a {@link KeyValueStore} based on the
 * filesystem.
 * </p>
 * <p>
 * Each key corresponds to a file in the filesystem, and thus one value per
 * file. A key names is translated to a file path by converting all "
 * <code>.</code>" characters to "<code>/</code>".
 * </p>
 * 
...
public final class FilesystemKeyValueStore implements KeyValueStore<byte[]> {
	...
	/**
	 * Converts the given <code>key</code> to a file, if the key is
	 * {@link KeyValueStoreHelper#isValidKey(String) valid}. Returns
	 * <code>null</code> if the key is invalid.
	 * 
	 * @throws IllegalArgumentException
	 *             If <code>rootDirectory</code> and/or <code>key</code> is null
	 */
	@Nullable
	public static File keyToFile(@NotNull final File rootDirectory, @NotNull final String key)
			throws IllegalArgumentException {
		...
		if (KeyValueStoreHelper.isValidKey(key)) {
			// convert dots to slashes and append to the root directory to
			// create a new File
	...
	/**
	 * Creates a new <code>FilesystemKeyValueStore</code> with the given
	 * <code>rootDirectory</code>. Throws an {@link IllegalArgumentException} if
	 * the given directory is <code>null</code> or does not exist.
	 * 
	 * @throws IllegalArgumentException
	 *             If <code>rootDirectory</code> is null
	 */
			...
			if (valueFile != null) {
				try {
					...
					if (valueFile.getParentFile().exists()) {
						...
						LOG.error("FilesystemKeyValueStore.set(): Failed to create the directories for value file ["
								+ valueFile + "]");
					}
				} catch (IOException e) {
	...
	/**
	 * Returns the {@link File} for the given <code>key</code> if and only if
	 * the key is non-<code>null</code>, valid AND denotes an existing key.
