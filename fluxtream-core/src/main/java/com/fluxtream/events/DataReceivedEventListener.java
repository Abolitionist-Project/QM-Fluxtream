...
/**
 * A Listener for Push Connector events whose handleEvent method calls an
 * external URL parameterized with the event's data The URL will have to contain
...
public class DataReceivedEventListener implements EventListener<DataReceivedEvent> {
	...
	public void handleEvent(final DataReceivedEvent event) {
		final StringBuilder msgAtts = new StringBuilder(
				"module=events component=DataReceivedEventListener action=handleEvent").append(" url=")
				.append(this.url);
		StringBuilder sb = new StringBuilder(msgAtts).append(" connector=").append(event.connector.getName())
				.append(" eventType=").append(event.objectTypes).append(" date=").append(event.date)
		...
		for (ObjectType objectType : event.objectTypes) {
			String invokeUrl = url.replaceAll("\\$guestId", String.valueOf(event.guestId))
					.replaceAll("\\$connectorName", event.connector.getName()).replaceAll("\\$date", event.date)
