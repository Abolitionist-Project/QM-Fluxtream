...
public class GuestServiceImpl implements GuestService {
	...
	private UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		final Guest guest = JPAUtils.findUnique(em, Guest.class, "guest.byUsername", username);
		if (guest == null) {
			return null;
		}
	...
	public UserDetails loadUserByEmail(String email) throws UsernameNotFoundException {
		final Guest guest = JPAUtils.findUnique(em, Guest.class, "guest.byEmail", email);
		if (guest == null) {
			return null;
		}
		return new FlxUserDetails(guest);
	...
	public Guest createGuest(String username, String firstname, String lastname, String password, String email)
			throws Exception {
	...
	@Override
	public Guest createGuest(String username, String firstname, String lastname, String password, String email,
			String oauthUserId, String oauthProviderName) throws Exception {
		if (getGuestByOauthCredentinals(oauthUserId, oauthProviderName) != null) {
			throw new Exception("User with such oauth credentials already exists");
		}
		Guest guest = new Guest();
		guest.username = username;
		guest.email = email;
		guest.firstname = firstname;
		guest.lastname = lastname;
		guest.oauthUserId = oauthUserId;
		guest.oauthProviderName = oauthProviderName;
		setPassword(guest, password);
		em.persist(guest);
		return guest;
	}
	...
	public Guest getGuest(String username) {
		return JPAUtils.findUnique(em, Guest.class, "guest.byUsername", username);
	}
	...
	@Override
	@Transactional(readOnly = false)
	public ApiKey setApiKeyAttribute(long guestId, Connector api, String key, String value) {
		ApiKey apiKey = JPAUtils.findUnique(em, ApiKey.class, "apiKey.byApi", guestId, api.value());
		if (apiKey == null) {
			apiKey = initializeApiKey(guestId, api);
		}
	...
	private ApiKey initializeApiKey(long guestId, Connector api) {
		ApiKey apiKey;
		apiKey = new ApiKey();
		apiKey.setGuestId(guestId);
		// set up connector with default hourly update interval
		apiKey.setConnector(api, Integer.parseInt(env.get("connectors.default_hourly_update_interval")));
		em.persist(apiKey);
		return apiKey;
	}
	...
	private void removeApiKeyAttribute(long guestId, ApiKey apiKey, String key) {
		List<ApiKeyAttribute> atts = JPAUtils.find(em, ApiKeyAttribute.class, "apiKeyAttribute.byKeyAndConnector",
				apiKey, key);
	...
	public void removeApiKey(long guestId, Connector connector) {
		ApiKey apiKey = JPAUtils.findUnique(em, ApiKey.class, "apiKey.byApi", guestId, connector.value());
		final String refreshTokenRemoveURL = apiKey.getAttributeValue("refreshTokenRemoveURL", env);
		// Revoke refresh token might throw. If it does we still want to remove
		// the apiKeys from
	...
	public List<ApiKey> getApiKeys(long guestId) {
		return JPAUtils.find(em, ApiKey.class, "apiKeys.all", guestId);
	}
	...
	public ApiKey getApiKey(long guestId, Connector api) {
		return JPAUtils.findUnique(em, ApiKey.class, "apiKey.byApi", guestId, api.value());
	}
	...
	public List<Guest> getAllGuests() {
		List<Guest> all = JPAUtils.find(em, Guest.class, "guests.all", (Object[]) null);
	...
	@Override
	public <T extends AbstractUserProfile> T getUserProfile(long guestId, Class<T> clazz) {
		CriteriaBuilder qb = em.getCriteriaBuilder();
		CriteriaQuery<T> c = qb.createQuery(clazz);
		Root<T> from = c.from(clazz);
		CriteriaBuilder criteriaBuilder = em.getCriteriaBuilder();
		Predicate guestPredicate = criteriaBuilder.equal(from.get("guestId"), guestId);
	...
	public ResetPasswordToken getToken(String token) {
		return JPAUtils.findUnique(em, ResetPasswordToken.class, "passwordToken.byToken", token);
	}
	...
	public Guest getGuestByEmail(String email) {
		return JPAUtils.findUnique(em, Guest.class, "guest.byEmail", email);
	}

	@Override
	public void deleteToken(String token) {
		ResetPasswordToken ptoken = JPAUtils.findUnique(em, ResetPasswordToken.class, "passwordToken.byToken", token);
		em.remove(ptoken);
	}
	...
	public void checkIn(long guestId, String ipAddress) throws IOException {
		...
		if (geoIpLookupService == null) {
			...
			geoIpLookupService = new LookupService(dbLocation, LookupService.GEOIP_MEMORY_CACHE);
		}
	...
	@Override
	public Guest getGuestByOauthCredentinals(String oauthUserId, String oauthProviderName) {
		return JPAUtils.findUnique(em, Guest.class, "guest.byOauthCredentinals", oauthUserId, oauthProviderName);
	}
