...
public interface ConnectorUpdateService {
	...
	/**
	 * Schedules an updated for on ObjectType of the given connector for the
	 * given user
	 * 
	 * @param guestId
	 *            The user for whom data is being collected
	 * @param connector
	 *            The connector that is being updated
	 * @param objectTypes
	 *            the objectType that is being updated. This is a bitmask which
	 *            can represent multiple objectTypes The value of each
	 *            objectType is defined in the ObjectType spec. Values are
	 *            always powers of 2 which allows for the bitmask. For example:
	 *            objectTypes = 5 means that both the objectType of value 4 and
	 *            the objectType of value 1 are to be updated
	 * @param force
	 *            force an update (sync now)
	 * @return A list containing data about what was scheduled
	 */
	...
	public List<ScheduleResult> updateAllConnectors(long guestId, boolean updateByScheduler);
	...
	public ScheduleResult scheduleUpdate(long guestId, String connectorName, int objectTypes,
			UpdateInfo.UpdateType updateType, long timeScheduled, String... jsonParams);
	...
	/**
	 * Returns a list of all scheduled updates for the connector for the given
	 * user NOTE: If a tasks has been running for over 10 hours, this method
	 * will set that tasks status to UpdateWorkerTask.Status.STALLED and will
	 * still return that result
	 * 
	 * @param guestId
	 *            the user whose status is being retrieved
	 * @param connector
	 *            The connector for which the tasks are being retrieved
