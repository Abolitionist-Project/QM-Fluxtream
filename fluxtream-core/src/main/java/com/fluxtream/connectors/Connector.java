...
public class Connector {
	...
	public String toString() {
		...
		if (this.objectTypes != null) {
			...
			for (int i = 0; i < objectTypes.length; i++) {
				if (i > 0)
					string += ", ";
				string += objectTypes[i].getName() + "/" + objectTypes[i].value();
			}
			string += "], objectTypeValues: [" + toString(objectTypeValues()) + "], objectTypeExtractorClasses: "
	...
	private AbstractFacetVOCollection getJsonFacetCollection(String name) {
		if (name == null)
			name = "default";
		Class<? extends AbstractFacetVOCollection> clazz = jsonFacetCollectionClasses.get(name);
		if (clazz == null)
			return null;
		try {
			return clazz.newInstance();
		} catch (Exception e) {
			throw new RuntimeException("Could not instantiate json facet collection: " + this.getClass().getName()
					+ "/" + name);
		}
	}
	...
	static {
		if (!initialized) {
			...
			for (BeanDefinition bd : scanner.findCandidateComponents("com.fluxtream.connectors"))
	...
	private static void extractConnectorMetadata(final BeanDefinition bd) {
		...
		Updater updaterAnnotation = connector.updaterClass.getAnnotation(Updater.class);
		...
		connector.updateStrategyType = updaterAnnotation.updateStrategyType();
		connector.hasFacets = updaterAnnotation.hasFacets();
		// set connectors' object types
		Class<? extends AbstractFacet>[] facetTypes = updaterAnnotation.objectTypes();
		...
		if (facetTypes.length == 1) {
			...
			if (updaterAnnotation.userProfile() != AbstractUserProfile.class)
				connector.userProfileClass = updaterAnnotation.userProfile();
		}
		...
		for (Class<? extends AbstractFacet> facetType : facetTypes) {
			ObjectTypeSpec ots = facetType.getAnnotation(ObjectTypeSpec.class);
			// objectTypes are mandatory only if there are more than 1
			if (ots == null) {
				if (facetTypes.length > 1)
					throw new RuntimeException("No ObjectTypeSpec Annotation for Facet [" + facetType.getName() + "]");
				else
					continue;
			}
			ObjectType objectType = new ObjectType();
			...
			if (ots.extractor() != null) {
				connector.addObjectTypeExtractorClass(objectType.value, ots.extractor(), ots.parallel());
			}
			connectorObjectTypes.add(objectType);
			ObjectType.addObjectType(ots.name(), connector, objectType);
		}
		if (connectorObjectTypes.size() > 0)
			connector.objectTypes = connectorObjectTypes.toArray(new ObjectType[connectorObjectTypes.size()]);

		JsonFacetCollection jsonFacetAnnotation = connector.updaterClass.getAnnotation(JsonFacetCollection.class);
		if (jsonFacetAnnotation != null)
			connector.jsonFacetCollectionClasses.put(jsonFacetAnnotation.name(), jsonFacetAnnotation.value());
	...
	private static Class<? extends AbstractUpdater> getUpdaterClass(String beanClassName) {
		Class<? extends AbstractUpdater> updaterClass = null;
		try {
			updaterClass = (Class<? extends AbstractUpdater>) Class.forName(beanClassName);
		} catch (Throwable t) {
			throw new RuntimeException("Could not get Updater Class for [" + beanClassName + "]");
		}
		return updaterClass;
	}

	private void addObjectTypeExtractorClass(int objectTypeValue,
			Class<? extends AbstractFacetExtractor> extractorClass, boolean parallel) {
		...
		if (!parallel && this.objectTypeExtractorClasses.containsValue(extractorClass)) {
			...
			this.objectTypeExtractorClasses.put(previousObjectType + objectTypeValue, extractorClass);
		} else
			this.objectTypeExtractorClasses.put(objectTypeValue, extractorClass);
	}
	...
	public int[] objectTypeValues() {
		if (this.objectTypeValues == null) {
			if (this.objectTypeExtractorClasses != null && this.objectTypeExtractorClasses.size() > 0) {
				Set<Integer> keySet = this.objectTypeExtractorClasses.keySet();
