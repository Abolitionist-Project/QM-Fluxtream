...
public class ApiClientSupport {
	...
	protected final boolean hasReachedRateLimit(Connector connector, long guestId) {
		String rateLimitString = (String) env.connectors.getProperty(connector.getName() + ".rateLimit");
		...
		if (rateLimitString.endsWith("/user")) {
			numberOfUpdates = connectorUpdateService.getNumberOfUpdatesSince(guestId, connector, then);

		} else {
			numberOfUpdates = connectorUpdateService.getTotalNumberOfUpdatesSince(connector, then);
		}
