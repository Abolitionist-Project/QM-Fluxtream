...
public class CalendarModel {
	...
	private static final DateTimeFormatter currentDateFormatter = DateTimeFormat.forPattern("EEE, MMM d, yyyy");
	private static final DateTimeFormatter jsDateFormatter = DateTimeFormat.forPattern("yyyy-MM-dd");
	private static final DateTimeFormatter shortDayFormatter = DateTimeFormat.forPattern("MMM d");
	private static final DateTimeFormatter currentMonthFormatter = DateTimeFormat.forPattern("MMMMMMMMMMMMM yyyy");
	private static final DateTimeFormatter currentYearFormatter = DateTimeFormat.forPattern("yyyy");
	...
	public static CalendarModel fromState(final long guestId, final MetadataService metadataService, final String state) {
	...
	public static LocalDate getBeginningOfWeek(final int year, final int week) {
		return (new LocalDate()).withWeekyear(year).withWeekOfWeekyear(week).minusWeeks(1)
				.withDayOfWeek(FIRST_DAY_OF_WEEK);
		// Need to subtract 1 week because Sunday is the last day of the week,
		// which
		// would logically move all the week start dates forward by 6 days.
		// Better
	 ...
	 * 
	 * This is similar to the behavior of Python's range function or Java's
	 * String.substring method, and it has the desirable property that the
	 * number of milliseconds between midnight on getToDate() and midnight on
	 * fromDate is equal to the number of milliseconds in a day times the number
	 * of days in the range described by TimeUnit (assuming there is no time
	 * zone change, leap second, or daylight savings time start/end in the
	 * middle).
	 */
	...
	private DateTimeZone getBrowserDateTimeZone() {
		// TODO: This doesn't match the implementation, even though it should be
		// right
	...
	private long getMillis(final LocalDate date) {
		return date.toDateTimeAtStartOfDay(DateTimeZone.forTimeZone(getTimeZone(date))).getMillis();
	}
	...
	public String toJSONString(final Configuration env) {
		// TODO: Include information in JSON that tells which calendar cells
		// should
	...
	private boolean isToday() {
		...
		return (timeUnit == TimeUnit.DAY) && (fromDate.getYear() == today.getYear())
