...
public abstract class AbstractFacet extends AbstractEntity {
	...
	/**
	 * A string representation of the tags for this facet. You should NEVER set
	 * this field directly. Instead, always use the {@link #addTags} method
	 * which sets both this and the {@link #tagSet} fields.
	 */
	@Lob
	public String tags;

	/**
	 * A {@link Set} representation of the tags for this facet. You should NEVER
	 * set this field directly. Instead, always use the {@link #addTags} method
	 * which sets both this and the {@link #tags} fields.
	 */
	...
	/**
	 * Parses the given space-delimited tags {@link String}, replacing illegal
	 * characters with an underscore, and adds them to this instance's
	 * {@link #tags} and {@link #tagSet} fields. One should ALWAYS use this
	 * method instead of directly setting the member fields.
	 * 
	...
	/**
	 * Returns an {@link Collections#unmodifiableSet(Set) unmodifiable Set} of
	 * the tags for this facet.
	 */
	...
	private static AbstractFacet getOldestOrLatestFacet(EntityManager em, Long guestId, Connector connector,
			ObjectType objType, String sortOrder) {
		Class facetClass;
		if (objType != null) {
		...
		Query query = em.createQuery("select facet from " + entity.name() + " facet where facet.guestId = " + guestId
				+ " order by facet.end " + sortOrder + " limit 1");
	...
	public static List<AbstractFacet> getFacetsBefore(EntityManager em, Long guestId, Connector connector,
			ObjectType objType, Long timeInMillis, Integer desiredCount) {
		...
		final Query query = em.createQuery("select facet from " + entity.name() + " facet where facet.guestId = "
				+ guestId + " and facet.start <= " + timeInMillis + " order by facet.start desc limit " + desiredCount);
		query.setMaxResults(desiredCount);
		return (List<AbstractFacet>) query.getResultList();
	}

	public static List<AbstractFacet> getFacetsAfter(EntityManager em, Long guestId, Connector connector,
			ObjectType objType, Long timeInMillis, Integer desiredCount) {
		final Class facetClass;
		if (objType != null) {
			facetClass = objType.facetClass();
		...
		final Entity entity = (Entity) facetClass.getAnnotation(Entity.class);
		final Query query = em.createQuery("select facet from " + entity.name() + " facet where facet.guestId = "
				+ guestId + " and facet.start >= " + timeInMillis + " order by facet.start asc limit " + desiredCount);
		query.setMaxResults(desiredCount);
		return (List<AbstractFacet>) query.getResultList();
	}
