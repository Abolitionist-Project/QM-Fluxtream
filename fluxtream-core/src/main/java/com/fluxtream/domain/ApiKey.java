...
@Entity(name = "ApiKey")
@NamedQueries({
		@NamedQuery(name = "apiKeys.byConnector", query = "SELECT apiKey FROM ApiKey apiKey WHERE apiKey.api=?"),
		@NamedQuery(name = "apiKeys.delete.all", query = "DELETE FROM ApiKey apiKey WHERE apiKey.guestId=?"),
		@NamedQuery(name = "apiKeys.delete", query = "DELETE FROM ApiKey apiKey WHERE apiKey.id=?"),
		@NamedQuery(name = "apiKeys.all", query = "SELECT apiKey FROM ApiKey apiKey WHERE apiKey.guestId=?"),
		@NamedQuery(name = "apiKey.count.byApi", query = "SELECT COUNT(apiKey) FROM ApiKey apiKey WHERE apiKey.api=?"),
		@NamedQuery(name = "apiKey.byApi", query = "SELECT apiKey FROM ApiKey apiKey WHERE apiKey.guestId=? AND apiKey.api=?"),
		@NamedQuery(name = "apiKey.byAttribute", query = "SELECT apiKey FROM ApiKey apiKey JOIN apiKey.attributes attr WHERE attr.attributeKey=? AND attr.attributeValue=?") })
public class ApiKey extends AbstractEntity {
	...
	@Expose
	@Index(name = "hours_update_interval")
	public Integer hourlyUpdateInterval;
	...
	public void setConnector(Connector api, int hoursUpdateInterval) {
		this.api = api.value();
		this.hourlyUpdateInterval = hoursUpdateInterval;
	}
	...
	public boolean equals(Object o) {
		if (!(o instanceof ApiKey))
			return false;
		ApiKey key = (ApiKey) o;
		return key.guestId == guestId && key.api == api;
	}
