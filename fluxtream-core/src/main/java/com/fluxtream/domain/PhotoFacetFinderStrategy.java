...
public interface PhotoFacetFinderStrategy {
	...
	List<AbstractFacet> findBefore(long guestId, Connector connector, ObjectType objectType, long timeInMillis,
			int desiredCount);

	List<AbstractFacet> findAfter(long guestId, Connector connector, ObjectType objectType, long timeInMillis,
			int desiredCount);
