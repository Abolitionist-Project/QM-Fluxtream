...
@NamedQueries({
		@NamedQuery(name = "settings.byGuestId", query = "SELECT settings FROM Settings settings WHERE settings.guestId=?"),
		@NamedQuery(name = "settings.delete.all", query = "DELETE FROM Settings settings WHERE settings.guestId=?") })
