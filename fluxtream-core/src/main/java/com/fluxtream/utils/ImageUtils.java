...
public final class ImageUtils {
	...
	private static final class GeolocationImpl implements Geolocation {
		...
		private GeolocationImpl(@NotNull final GpsDirectory gpsDirectory) {
			...
			latitude = (latitudeRationals == null) ? null : GeoLocation.degreesMinutesSecondsToDecimal(
					latitudeRationals[0], latitudeRationals[1], latitudeRationals[2], "S".equals(latitudeRef));
			longitude = (longitudeRationals == null) ? null : GeoLocation.degreesMinutesSecondsToDecimal(
					longitudeRationals[0], longitudeRationals[1], longitudeRationals[2], "W".equals(longitudeRef));
	...
	/**
	 * Tries to read the given <code>imageBytes</code> and returns
	 * <code>true</code> if it's a GIF, JPEG, or PNG image; returns
	 * <code>false</code> otherwise. Returns <code>false</code> if the given
	 * byte array is <code>null</code> or empty.
	 */
	...
	/**
	 * Tries to create a JPEG thumbnail of the given image with the given
	 * desired dimensions. Returns <code>null</code> if the given byte array is
	 * <code>null</code> or empty, if the desired
	 * <code>lengthOfLongestSideInPixels</code> is zero or negative, or if the
	 * bytes cannot be read as an image. This method will attempt to read
	 * orientation info from the EXIF and, if necessary, rotate/flip the
	 * thumbnail appropriately. Note that if the image has an alpha channel, it
	 * will be discarded before generating the thumbnail.
	 * 
	 * @throws IOException
	 *             if a problem occurs while reading the image or generating the
	 *             thumbnail
	 */
	@Nullable
	public static Image createJpegThumbnail(@Nullable final byte[] imageBytes, final int lengthOfLongestSideInPixels)
			throws IOException {
		if (imageBytes != null && imageBytes.length > 0 && lengthOfLongestSideInPixels > 0) {
			...
			try {
				...
				if (image != null) {
					...
					return JpegImage.create(orientation.transform(Scalr.resize(image, Scalr.Method.AUTOMATIC,
							Scalr.Mode.AUTOMATIC, lengthOfLongestSideInPixels)));
				}
			} catch (Exception e) {
	...
	// I stole this from:
	// https://github.com/thebuzzmedia/imgscalr/issues/82#issuecomment-11776976
	@NotNull
	private static BufferedImage dropAlphaChannel(@NotNull final BufferedImage srcImage) {
		final BufferedImage convertedImg = new BufferedImage(srcImage.getWidth(), srcImage.getHeight(),
	...
	@Nullable
	public static Geolocation getGeolocation(@Nullable final byte[] imageBytes) throws ImageProcessingException,
			IOException {
		if (imageBytes != null && imageBytes.length > 0) {

			final Metadata metadata = ImageMetadataReader.readMetadata(new BufferedInputStream(
					new ByteArrayInputStream(imageBytes)), false);
			if (metadata != null) {
	...
	/**
	 * Returns the {@link ImageType} of the given image, or <code>null</code> if
	 * the type is unknown, not supported, or if an error occurs while trying to
	 * read the image. Returns <code>null</code> if the given image byte array
