package com.fluxtream.test.utils;
...
public class ImageUtilsTest {
	...
	public void testIsSupportedImage() {
		...
		Assert.assertFalse(ImageUtils.isSupportedImage(IMAGE_5)); // TIFFs are
																	// not
																	// supported
		Assert.assertTrue(ImageUtils.isSupportedImage(IMAGE_6));
		Assert.assertFalse(ImageUtils.isSupportedImage(NOT_AN_IMAGE)); // not
																		// actually
																		// an
																		// image
	}
	...
	public void testCreateThumbnail() throws Exception {
		...
		testCreateThumbnailHelper(IMAGE_1,
				(int) Math.max(IMAGE_1_EXPECTED_SIZE.getHeight(), IMAGE_1_EXPECTED_SIZE.getWidth()),
				IMAGE_1_EXPECTED_SIZE);

		testCreateThumbnailHelper(IMAGE_2, 50, new Dimension(38, 50));
		testCreateThumbnailHelper(IMAGE_2, 100, new Dimension(75, 100));
		testCreateThumbnailHelper(IMAGE_2,
				(int) Math.max(IMAGE_2_EXPECTED_SIZE.getHeight(), IMAGE_2_EXPECTED_SIZE.getWidth()),
				IMAGE_2_EXPECTED_SIZE);

		testCreateThumbnailHelper(IMAGE_3, 50, new Dimension(38, 50));
		testCreateThumbnailHelper(IMAGE_3, 100, new Dimension(75, 100));
		testCreateThumbnailHelper(IMAGE_3,
				(int) Math.max(IMAGE_3_EXPECTED_SIZE.getHeight(), IMAGE_3_EXPECTED_SIZE.getWidth()),
				IMAGE_3_EXPECTED_SIZE);

		testCreateThumbnailHelper(IMAGE_4, 50, new Dimension(47, 50));
		testCreateThumbnailHelper(IMAGE_4, 100, new Dimension(93, 100));
		testCreateThumbnailHelper(IMAGE_4,
				(int) Math.max(IMAGE_4_EXPECTED_SIZE.getHeight(), IMAGE_4_EXPECTED_SIZE.getWidth()),
				IMAGE_4_EXPECTED_SIZE);

		testCreateThumbnailHelper(IMAGE_6, 50, new Dimension(43, 50));
		testCreateThumbnailHelper(IMAGE_6, 100, new Dimension(85, 100));
		testCreateThumbnailHelper(IMAGE_6,
				(int) Math.max(IMAGE_6_EXPECTED_SIZE.getHeight(), IMAGE_6_EXPECTED_SIZE.getWidth()),
				IMAGE_6_EXPECTED_SIZE);
	}

	private void testCreateThumbnailHelper(@NotNull final byte[] imageBytes, final int lengthOfLongestSideInPixels,
			@NotNull final Dimension expectedDimension) throws IOException {
	...
	public void testConversion() throws Exception {
		...
		// TIFFs apparently may or may not be supported, depending on platform,
		// so no null assert here.
